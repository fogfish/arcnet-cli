...
Filtering; the filter never excludes the seed itself). --predicate <name>
restricts which structural connections are followed during expansion to
the named relation(s), independent of --type/--tag/--attr, which narrow
the result instead. The result is serialized as one patch-exchange
document, grouped by type, ready to re-ingest via arc apply or paste into
an LLM prompt. subgraph is strictly read-only.
