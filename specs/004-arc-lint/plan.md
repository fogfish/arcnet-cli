...
**Bugfix**: 2026-07-04 — BUG-001: `checkDerivedProvenance` reported a false-positive `derivedProvenance` violation against every `timeline`-kind period file (yearly and monthly) — spec.md FR-006's own wording over-generalized CORE §3.4's "a node distilled from a document" to every non-`source` kind, incorrectly including the tool's own reserved `timeline` index kind, which is never content synthesized from one document. See `research.md` D8's new Bugfix paragraph for the revised decision (exempt `timeline` alongside `source`) and the clarified spec.md FR-006. No task reopened — `checkDerivedProvenance` (T033) correctly implemented FR-006 as originally written; new tasks T057-T058 added (see `tasks.md`). A second, independent gap was noted during investigation but not fixed here (out of scope for this bug): `internal/core.ParseNode`'s list-item link grammar cannot recognize `internal/app/graph/service.applyTimeline`'s own rendered bullet as a link at all, since it requires the whole list item to be exactly `[[Target]]` — recorded in `research.md` D8 for any future rule that might need to resolve a `timeline` node's own links.
