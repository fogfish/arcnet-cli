...
**Bugfix**: 2026-07-04 — BUG-001: The original decision exempted only `source`, over-generalizing CORE §3.4's "a node distilled from a document" to every other kind, including the format's own reserved `timeline` index kind (`internal/core.CoreMergeRules`'s fourth fixed kind, alongside `source`/`entity`/`resource`) — a timeline period file is the tool's own chronological index referencing many documents over time, never content synthesized from one document, and CORE §12.2 already excludes it from ordinary patch-carried content (`specs/003-apply-patch/research.md` D8b). Revised: `checkDerivedProvenance` now also exempts `Kind == "timeline"`, the same way it already exempts `Kind == "source"`.

Separately, and independent of this fix: `internal/core.ParseNode`'s list-item link grammar (`listItemPattern`) requires an entire list item to be exactly `[[Target]]` (or `predicate:: [[Target]]`) to recognize it as a link — `internal/app/graph/service.applyTimeline`'s own rendered bullet (`- [[id]] — *title* (author) — date`, via `core.TimelineEntry`) has trailing content after the link and is therefore never recognized as a `Link` by any generic, `core.Node`-based consumer, `arc lint` included. This fix does not depend on that gap ever being closed — exempting `timeline` from `checkDerivedProvenance` entirely means lint never needs to resolve its links in the first place — but it is recorded here so a future rule that *does* need to inspect a `timeline` node's own recorded references (none exists today) does not rediscover this the hard way.
