...
- **FR-006**: For every derived node (any node distilled from a document, i.e. every node other than a `source` node itself, **excluding the format's own reserved `timeline` index kind**), the tool MUST verify it links to at least one `source` node, reporting a violation with file path when no such link is found. *(Clarified — BUG-001: a `timeline` period file is the tool's own chronological index over many documents, not content "distilled from a document" — CORE §12.2 already excludes it from ordinary patch-carried content, and it MUST be exempt from this check the same way a `source` node itself is.)*
...
**Bugfix**: 2026-07-04 — BUG-001: Clarified FR-006 to exempt the format's reserved `timeline` index kind from the derived-provenance check, alongside the existing `source` exemption. `checkDerivedProvenance` correctly implemented FR-006 exactly as written; the requirement's own parenthetical ("every node other than a `source` node itself") over-generalized CORE §3.4's actual scope ("a node distilled from a document") to include the tool's own chronological index over many documents, which is categorically not that. Separately (not itself a spec change, but recorded for traceability): `internal/core.ParseNode`'s list-item link grammar cannot recognize `internal/app/graph/service.applyTimeline`'s own rendered bullet (`- [[id]] — *title* (author) — date`) as a link at all, since it requires the entire list item to be exactly `[[Target]]` — this means a `timeline`-kind node's own recorded references remain invisible to any generic, `core.Node`-based consumer regardless of this fix; the FR-006 exemption sidesteps the practical symptom (a false-positive violation) without depending on that separate parsing gap ever being closed.
