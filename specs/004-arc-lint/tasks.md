...
**Bugfix**: 2026-09-03 — 016-arc-revert BUG-002 — cross-reference, no task reopened here. `CommitsMatching` (T015/T016/T020) issues `git log --all --grep=<needle>` with no pathspec, so FR-010's one-`graph(ingest):`-commit-per-document check searches the entire enclosing repository's history: a second graph sharing the repository that ingested the same `Source-Id` is wrongly counted toward this document's commit total. T015/T016 are not reopened — the defect is in the shared adapter's argv, which specs/016-arc-revert/tasks.md T047 corrects; lint-side regression coverage for a nested graph is T051 there. data-model.md's port section is corrected in place.
