...
3. **Given** a node that was originally created as a stub (kind and id only, no `published` set), **When** a later patch merges real content into it for the first time, **Then** the merge fills in `published` the same way it fills in any other previously-empty attribute under that kind's merge rules, and the node also receives `updated` for this application — but its original `indexed` value (set when the stub was first created) is left unchanged.
...
- What happens when a single patch application both creates some nodes and merges into others? Every node it creates gets the same `indexed` value, and every node it actually merges into (excluding "none"-behavior no-ops) gets the same `updated` value — and both values are identical to each other, since both describe the same single application.
...
- **FR-007**: When a patch's contribution to an already-existing node produces an actual content change under that node kind's declared merge behavior, the tool MUST set that node's `updated` attribute to an ISO 8601 timestamp at second resolution.
- **FR-008**: The tool MUST NOT set `updated` on a node whose merge behavior is "none," since a second contribution to that kind is already required (spec 003 FR-007) to leave the file completely unchanged.
...
- **Merged Node**: An already-existing node whose file content actually changed as a result of a patch's contribution being combined into it per that kind's declared merge behavior; excludes "none"-behavior no-ops, which remain untouched.
...
- **SC-002**: 100% of nodes actually changed by a merge during a patch application carry `updated`, using the exact same timestamp value as every other node — created or merged — touched by that same application.
...
- **SC-004**: 100% of "none"-merge-behavior nodes re-contributed by a later patch remain byte-for-byte unchanged, with zero instances of an `updated` attribute being added.
