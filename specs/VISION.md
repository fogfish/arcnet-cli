...
> **Terminology note.** This document predates ARCNET-CORE v0.5 and still speaks of a node's `kind` and of a `_meta/` registry. Both were superseded: a node's category is its `"@type"`, and the registry is the first-class `_schema/` tree — `_schema/Property/<name>.md` per predicate, `_schema/Class/<Name>.md` per type. Entries corrected against shipped behaviour are marked; unmarked roadmap entries still carry the older vocabulary and are corrected as they are implemented. The authoritative description of what ships today is [README.md](../README.md) and each feature's own `specs/<nnn>-*/spec.md`.

`arc` is **extension-agnostic**: domain profiles (ARCNET-DOMAIN-*) add new `"@type"` values and predicates, but `arc` handles them transparently through the core mechanisms — **per-predicate** merge operations declared on each predicate's own `_schema/Property/` document, the schema index, and the alias table. Merge is a property of a predicate, never of a type: CORE §9.3 retired type-level merge, and its menu is closed at six (`immutable`, `union`, `firstWriteWin`, `fillIfEmpty`, `lastWriteWin`, `append`). No special subcommand exists for any extension.
...
- [x] `arc init [<dir>]` — initialize a new knowledge graph: create the canonical folder layout — the type folders `Source/`, `Entity/`, `Resource/`, `Reference/`, each named for its type character for character (CORE §6 v0.11), plus `timeline/yearly/`, `timeline/monthly/`, and the two schema folders `_schema/Class/` and `_schema/Property/`; seed `_schema/` with ARCNET-CORE v0.11's full built-in vocabulary, one document per type and per predicate; create `.arc/` for arc-managed state (see Graph Root); run `git init` and create `.gitkeep` for empty folders; write `.gitignore` excluding `.arc/`; stage everything and produce the initial commit `graph(init): empty knowledge graph` (CORE §13.3)
- [x] `arc upgrade [--dry-run]` — bring an existing graph's built-in vocabulary up to date with the installed release, in exactly one `graph(migrate):` commit: replace every built-in `_schema/` document **outright** (every correction is a retraction the additive merge path cannot express), delete any this release no longer seeds, and leave every author-written `_schema/` document and every file outside `_schema/` untouched; report content nodes whose first-fixed prose shows evidence of accumulation under an older merge behaviour, never repairing them; no change means no commit, so repeat runs are safe. Deliberately writes the corrected vocabulary **before** reading anything, so it can run on a graph whose own schema no longer validates
...
- [x] `arc apply <patch.md>` — apply a patch file to the graph (CORE §12.3): parse the patch manifest (`"@type": patch`, `document`, `published`, `stats`); check idempotency and skip with a clear message if `sources/<id>.md` is already tracked (CORE §11.2); for each H1/H2 node section reconstruct the node object (ARCNET-AST §4); **create** new node files when the basename does not exist; **merge** into existing files per **each predicate's own** declared merge operation, looked up in the schema index — never one behaviour applied to the whole node (CORE §9.3); type-specific prose (`abstract`, `definition`, `relevance`, `description`) is first-fixed, so a divergent contribution preserves the established value and is flagged for review rather than appended; derive and append timeline entries from the source's `published` date (CORE §9.4); produce exactly one git commit with the mandatory subject, stats, and `Source-Id:` trailer (CORE §11.3); update the local index cache (Phase 4) atomically within the same filesystem transaction
...
- [ ] `arc serve` — start an MCP server (stdio transport by default; `--http <port>` for SSE) exposing these tools:
  ...
  - [x] `subgraph_get(id, depth?)` → return the fully-resolved subgraph rooted at `id` to `depth` hops (default 1): a flat array of complete node objects for the seed and every reachable neighbor; covers the same operation as `arc subgraph` for agent context expansion mid-conversation
...
- **Predicate and alias registry management (#9)** — There is no CLI for adding a single new predicate document under `_schema/Property/` or a new alias to `_meta/aliases.md`. Predicates arrive today by hand, by `arc apply`'s auto-registration, or in bulk via `arc apply schema`; the built-in set is replaced wholesale by `arc upgrade`. Explicit `arc predicate add` and `arc alias add` commands are still needed for safe, validated curation of a single entry.
