...
- [ ] `arc serve` — start an MCP server (stdio transport by default; `--http <port>` for SSE) exposing these tools:
  ...
  - [x] `context_retrieve(query, filter?, limit?)` → the primary RAG tool: runs the same three-pass retrieval as `arc context` — grep match, attribute match, neighbor expansion — and returns the top-N node objects with full content (attrs + text + edges + links); designed to let an agent build its working context in a single tool call without iterating through grep results and fetching each node separately; `limit` defaults to 10
...
- **Predicate and alias registry management (#9)** — There is no CLI for adding a single new predicate document under `_schema/Property/` or a new alias to `_meta/aliases.md`. Predicates arrive today by hand, by `arc apply`'s auto-registration, or in bulk via `arc apply schema`. Explicit `arc predicate add` and `arc alias add` commands are still needed for safe, validated curation of a single entry.
