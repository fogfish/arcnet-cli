...
- Third-party GitHub Actions pinned by this feature (`actions/checkout`, `actions/setup-go`, `shogo82148/actions-goveralls`, `dominikh/staticcheck-action`, `reecetech/version-increment`, `goreleaser/goreleaser-action`) are assumed to track GitHub's currently supported Actions runtime (e.g., Node 24) on a best-effort basis; a pinned action falling behind (surfacing as a runner deprecation warning, not a failure) is an accepted, upstream-owned maintenance concern to be periodically checked, not something this repository can force any given action's maintainer to fix (see BUG-002).
...
**Bugfix**: 2026-07-01 — BUG-002 Added assumption that pinned third-party Actions are only expected to track GitHub's supported Actions runtime on a best-effort basis (all three workflows were emitting a non-blocking "Node 20 is being deprecated" runner warning; no FR previously addressed Actions-runtime currency, only the Go toolchain pin in FR-011).
