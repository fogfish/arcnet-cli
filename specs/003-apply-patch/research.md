...
type Node struct {
    ...
    Text  string          // plain prose, wikilink brackets stripped — see D3b
    Notes string          // plain prose, wikilink brackets stripped — see D3b
    HRefs []Link
    Edges []Link
...
## D3b: Rendering `HRefs` back into `Text`/`Notes` (inline wikilink reconstruction)

**Added 2026-07-02**, per explicit user direction (marked important). `HRefs` is parsed from a node's prose purely for internal navigation (D3, AST §6.3 "Inline Link Cache") — but because D3 *strips* the `[[...]]` bracket markup out of `Text`/`Notes` when it records the link into `HRefs`, `RenderNode` must be able to put that markup back when it serializes a node — otherwise a round-tripped node would silently lose its inline wikilinks.

**Decision**: `RenderNode` reconstructs inline wikilink markup in `Text`/`Notes` from `HRefs`, in `HRefs` order (AST invariant 4: item order is preserved and significant), by locating the display substring — `Alias` if the `Link` has one, else `Target` — inside the plain text and wrapping the **first not-yet-consumed** matching occurrence in brackets (`[[Target]]`, or `[[Target|Alias]]` when `Alias` is set, or the inline-predicate form `[predicate:: [[Target]]]` when `Predicate` is set). A candidate occurrence is accepted only when **both**:

1. **It is not already inside brackets.** The scan tracks bracket depth/spans already produced by an earlier `HRefs` entry in the *same* render pass (and any literal `[...]`/`[[...]]` span that happened to already exist in the plain text, e.g. a stray citation-style `[1]`) and skips any candidate match that falls inside one — this is what prevents `[[Target]]` being re-wrapped into `[[[[Target]]]]`, or a substring inside one link's brackets being mistaken for a second, nested link.
2. **It does not start mid-word.** A match is only applied when it begins at the very start of `Text`/`Notes` or is immediately preceded by a whitespace character — never immediately preceded by a letter/digit/other word character. This is the literal rule as specified ("wikilinks must be applied if matching text starts with whitespace"); by the same logic (word-boundary safety must hold on both sides of a match to genuinely avoid "the middle of a word"), a candidate match is symmetrically rejected if it does not *end* at a whitespace character, punctuation, or the end of the text — this trailing-boundary check was not spelled out explicitly in the instruction but is a direct, necessary consequence of it, and is called out here rather than silently assumed.

If a href's display substring is not found anywhere satisfying both rules (e.g. the original author phrased the mention differently than the stored `Target`/`Alias`, or the eligible occurrence was already consumed by an earlier, identically-named href), `RenderNode` leaves it unlinked in the rendered text rather than forcing an unsafe match — the `Link` itself is not dropped; `HRefs` is still written into the node's JSON-equivalent shape wherever `RenderNode`'s caller needs it (this only affects the *inline bracket reconstruction inside prose*, never `HRefs` itself as data).

**Known, flagged tension with AST §3.6's "lossless conversion" invariant**: substring-based reconstruction is a *best-effort, deterministic* re-linking, not a guaranteed byte-for-byte round trip. If a node's original prose mentions the same target name twice — once meant as a link, once as incidental plain text — parsing collapses both into identical bracket-free substrings, and rendering can only re-link *an* occurrence (the first eligible one), not necessarily and only the one originally bracketed. The rendered *content* remains correct in reading order and meaning; only exact positional fidelity in this specific repeated-mention scenario is not guaranteed. This is accepted as the practical cost of storing `Text`/`Notes` free of bracket noise (so, e.g., a future `arc lint`/diff feature can compare prose without link-syntax churn), flagged here rather than silently claimed as fully lossless.

**Rationale**: This is the design the user specified directly, correcting an implicit assumption in the original D2/D3 draft (that `Text`/`Notes` retained bracket markup verbatim, with `HRefs` as a pure side-channel cache alongside it — the AST spec's own §6.1 wording is compatible with either reading, since it never explicitly states whether the brackets themselves survive inside `text`). Stripping brackets from `Text`/`Notes` and treating `HRefs` as the sole source of "where a link goes" keeps prose comparisons (merge conflict detection, D6; future diff/lint tooling) free of link-syntax noise, at the acknowledged cost of the best-effort (not byte-exact) reconstruction noted above.

**Alternatives considered**: Keeping `Text`/`Notes` bracket-inclusive verbatim, with `HRefs` as a pure cache never re-rendered — rejected per the user's explicit instruction; also would have made `RenderNode` trivial (no reconstruction needed at all), but that is exactly not what was asked for. Rejecting a match only for the leading-word-boundary case (as literally worded) without the trailing check — rejected as internally inconsistent with the stated goal ("preventing application... in the middle of a word"), which a leading-only check does not fully achieve (see decision text above).
