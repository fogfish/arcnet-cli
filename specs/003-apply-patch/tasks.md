...
## Bugfix: BUG-005 — a patch-carried `kind: timeline` section lands in a bogus `timelines/` folder, colliding with the tool's own derived timeline index

**Purpose**: Fix `specs/003-apply-patch/bugs/BUG-005.md`. **Superseded same-day by BUG-006 below** — this fix's "refuse the whole patch" approach was itself wrong; see `research.md` D8b's revised decision and the "Bugfix: BUG-006" section below for the corrected implementation.

- [X] T097 ⚠️ Reopened — BUG-006 [US1] ~~Implement the reserved-kind guard in `internal/core/markdown.go`'s `parsePatchBody`: reject a patch whose body contains an H1 section naming the literal, case-sensitive kind `timeline`, returning `ErrPatchStructure`~~ — reverted; `ParsePatch` no longer refuses a `kind: timeline` H1 section, see T102
- [X] T098 ⚠️ Reopened — BUG-006 [P] ~~Add a unit test asserting `ParsePatch` returns `ErrPatchStructure` for a `# Timeline` H1 section~~ — replaced with a test asserting the section parses successfully, see T103
- [X] T099 ⚠️ Reopened — BUG-006 [P] [US1] ~~Add an E2E test asserting `arc apply` refuses a patch declaring a `# Timeline` section~~ — replaced with tests asserting such a patch applies successfully, see T104/T105
- [X] T100 [P] Update `contracts/ast-contract.md`'s `ParsePatch` error-contract bullet — bullet reverted again by BUG-006 (see T102's note); task retained for traceability (depends on T097)
- [X] T101 [P] Optional, defense-in-depth only — superseded: `nodeFolder`'s doc comment was revised again to describe the corrected mechanism (Apply's per-node loop intercepts `Kind == "timeline"` before `nodeFolder` is ever consulted, see T102), not the original parse-time refusal

**Checkpoint**: BUG-005's diagnosis (the folder-collision mechanism) was correct; its fix (refuse the whole patch) was not — see BUG-006 below for the corrected resolution

---

## Bugfix: BUG-006 — BUG-005's "refuse the whole patch" fix broke real patches that legitimately carry a `kind: timeline` section

**Purpose**: Fix `specs/003-apply-patch/bugs/BUG-005.md`'s own incorrect resolution (reported same-day against a real patch from `github.com/fogfish/bots`). See `research.md` D8b's revised decision, `contracts/ast-contract.md`'s reverted `ParsePatch` error-contract bullet, and the revised spec.md FR-025/SC-010/Edge Case bullet.

- [X] T102 [US1] Revert T097's guard in `internal/core/markdown.go`'s `parsePatchBody` (a `kind: timeline` H1 section is valid patch structure again); in `internal/app/graph/service/apply.go`'s `Apply`, intercept `node.Kind == "timeline"` at the top of the per-node loop — collect its `node.ID` (a declared period code) into a new `timelinePeriodsFromPatch []string`, emit a `reporter.Step` line, and `continue` (never call `nodePath`/`readExistingNode`/`core.Merge`/`writeNode` for it) (research.md D8b revised)
- [X] T103 [US1] Extend `internal/app/graph/service/apply.go`'s `applyTimeline` to accept `extraPeriods []string`: for each, if it matches the monthly shape (`YYYY-MM`) touch both that monthly period and its yearly rollup (leading `YYYY`); if it matches the yearly shape (`YYYY`) touch only that yearly period; reuse the existing `upsertTimelinePeriod` (already idempotent per-document-id) for every touched period via a new `periodGranularity(period string) (path, granularity, heading string, ok bool)` helper (depends on T102)
- [X] T104 [P] Replace T098's reverted test in `internal/core/markdown_test.go` with `TestParsePatchTimelineKindSectionParsesAsOrdinaryNode`, asserting `ParsePatch` succeeds and the timeline section appears in `patch.Nodes` with `Kind == "timeline"` (constitution Principle VI) (depends on T102)
- [X] T105 [P] [US1] Replace T099's reverted test in `cmd/arc/graph/apply_test.go` with two E2E tests: `TestApplyPatchCarriedTimelineSectionFoldedIntoIndex` (a patch with both `# Source` and `# Timeline` sections applies successfully, no `timelines/` folder is created, and both the correct monthly and yearly period files contain the document) and `TestApplyPatchCarriedTimelineSectionCascadesToYearlyForDifferentMonth` (the patch's declared period differs from the month `patch.Published` itself derives — both monthly periods end up populated and the shared yearly rollup contains the entry exactly once, not duplicated) (depends on T103)
- [X] T106 [P] Update `contracts/ast-contract.md`'s `ParsePatch` error-contract bullet: remove the reserved-kind `ErrPatchStructure` case T100 added; state that `kind: timeline` parses like any other section and it is `service.Apply`'s responsibility (not `ParsePatch`'s) to fold it into the timeline index (research.md D8b revised) — already done as part of this bugfix patch; task retained for traceability (depends on T102)

**Checkpoint**: BUG-006 resolved — a patch carrying both a `# Source` and an explicit `# Timeline` section (the real, previously-failing case) applies successfully; the declared period is folded into the tool's own timeline index with no `timelines/` collision and no format-incompatible duplicate file; a monthly-only declared period also updates its yearly rollup

---

## Bugfix: BUG-007 — a yearly timeline period file fails `core.ParseNode` because YAML decodes its bare `period` value as an integer, not a string

**Purpose**: Fix `specs/003-apply-patch/bugs/BUG-007.md`. Discovered via `arc lint` (specs/004-arc-lint), not `arc apply` itself. See `research.md` D8's new Bugfix paragraph for the revised decision and the clarified spec.md FR-010.

- [X] T107 [US1] In `internal/app/graph/service/apply.go`'s `upsertTimelinePeriod`, write the `period` front-matter field as an explicitly-quoted YAML string (`period: "` + period + `"`) instead of a bare, unquoted scalar — guarantees it decodes as a string regardless of lexical shape, matching what `internal/core.RenderNode`'s YAML-safe encoder would already produce for any other node file's front matter (research.md D8 Bugfix, spec.md FR-010) (depends on `upsertTimelinePeriod`'s existing implementation, T063)
- [X] T108 [P] Add a unit test in `internal/app/graph/service/apply_test.go` (or an E2E test in `cmd/arc/graph/apply_test.go`) asserting a yearly timeline period file (bare 4-digit `period` value) round-trips successfully through `internal/core.ParseNode` after `arc apply` writes it — constitution Principle VI; guards against this exact regression recurring (depends on T107)

**Checkpoint**: BUG-007 resolved — every timeline period file `arc apply` writes (yearly and monthly) parses successfully via the generic `core.ParseNode`, not only via `upsertTimelinePeriod`'s own bespoke read path

---

## Bugfix: BUG-008 — a node identity differing only in letter case aborts the apply on a case-insensitive volume, and forks the graph on a case-sensitive one

**Purpose**: Fix `specs/003-apply-patch/bugs/BUG-008.md`. A spec gap, not implementation drift — `nodePath`, `readExistingNode` and `validateNodeBasename` each correctly implemented what the spec asked for; FR-005/FR-006 never defined what "identity already exists" means on a storage location that folds letter case, so the filesystem was implicitly deciding it. See `research.md` D11 for the new decision and the new spec.md FR-026/FR-027/FR-028/FR-029, SC-011/SC-012, Edge Case bullet and Out of Scope bullet.

**No task reopened** — see the Purpose above.

- [X] T109 [US1] **Prerequisite investigation, blocks T111-T113** — establish empirically whether `internal/adapter/git`'s `IsTracked` (`git ls-files --error-unmatch`) folds letter case under `core.ignorecase=true`, and record the answer in `research.md` D11's "Deliberately unresolved" paragraph, converting it to a decision. This is a git path query with its own semantics, independent of the filesystem probe: if it folds case, two patches whose `document` ids differ only in case are both ingestible today, defeating spec FR-003's idempotency guarantee on exactly the volumes where T111's fold prevents node-level duplication. Do not assume a direction — run it (research.md D11, spec.md FR-003). **Answer: exact-match, always** — `core.ignorecase` does not affect `ls-files`, which matches the index's exact byte paths. This widens T111's scope: the same case-resolution MUST cover the `Source/<document>.md` idempotency question, or the fold silently converts today's hard abort into a double-ingest. See D11's resolved paragraph.
- [X] T110 [US1] Add the letter-case probe to `internal/adapter/fsys`: create a throwaway file under the graph's already-gitignored `.arc/`, look it up under a flipped-case name **via `ReadDir` on the parent, never `Stat`** (`os`'s `FileInfo.Name()` is derived from the path string passed in, so `Stat` reports the requested spelling and would always appear to match — research.md D11's "`Stat` trap"), remove the probe file on every path including failure, and cache the result on the `Store` that `Mounter.Mount` returns so it is established once per invocation and inherited by every path-resolving caller. On probe failure, report the location as case-**insensitive** — the branch that refuses to fork the graph (spec.md FR-026)
- [X] T111 [US1] In `internal/app/graph/service/apply.go`, resolve the incoming identity to the spelling already on disk before `nodePath`'s result reaches `readExistingNode`/`writeNode`, for graph locations the T110 probe reports as case-insensitive only: an incoming identity differing from an existing node's identity only in letter case merges into that existing file under the identity already recorded there, which stays authoritative (`core.Merge`'s `merged := existing` already delivers this half — research.md D6/D11). Where the probe reports case-sensitive, behavior is unchanged and comparison stays exact (spec.md FR-027, depends on T110; blocked by T109)
- [X] T112 [US1] Append a warning to `kernel.ApplyResult.Warnings` naming both the incoming spelling and the canonical identity it was folded into, surfaced the same way the existing unrecognized-type warning is (`apply.go`'s per-node loop) — the fold MUST NOT be silent (spec.md FR-027, depends on T111)
- [X] T113 [US1] Correct `validateNodeBasename` in `internal/app/graph/service/apply.go` to compare a parsed `@id` against the name the store reports for the file it **actually opened** (resolved via `ReadDir`, per T110's note), never against `filepath.Base` of a caller-constructed path string. Preserve its genuine purpose — catching a hand-edited node file whose `@id` drifted from its own filename — and leave its true-positive call site inside `guardNoOldFormatNodes` (which already walks real directory entries via `walkNodeFiles`) working unchanged (spec.md FR-029, depends on T110; blocked by T109)
- [X] T114 [P] Unit tests in `internal/app/graph/service/apply_test.go` against the existing `fsys.Store` fake, covering **both** probe outcomes deterministically by injecting the probe result rather than depending on the test machine's own disk (spec.md SC-012): case-sensitive ⇒ `LightStep` and `Lightstep` produce two distinct files, compared exactly; case-insensitive ⇒ one file, merged under the recorded `@id`, with the T112 warning present. Assert on the **exact path string handed to the store**, never on a filesystem stat (`specs/022-reference-type-folders` contract C7) — constitution Principle VI (depends on T111, T112)
- [X] T115 [P] [US1] E2E test in `cmd/arc/graph/apply_test.go` reproducing this bug's own two-patch fixture on the `tls13Patch`/`initGraph` helpers: patch A carries entity id `LightStep`, patch B (different `document` id) carries `Lightstep`. Assert the second apply succeeds rather than aborting, that the graph ends up with exactly one file for the subject still carrying its originally recorded `@id`, that the T112 warning names both spellings, and that no diagnostic names a nonexistent file — spec.md SC-011, constitution Principle VI; guards this exact regression (depends on T111, T113)
- [X] T116 [P] Extend `specs/022-reference-type-folders/contracts/folder-layout-contract.md`'s C7 ("compare the exact path string the store was asked for; never rely on a filesystem stat") from type-**folder** names to node **identities**, cross-referencing BUG-008 — the hazard class 022 already reasoned about for folders, never applied to identities, which is precisely this bug's gap (depends on T113)
- [X] T117 [P] Add a regression test confirming this fix's scope, guarding spec.md FR-028: an identity is never normalized graph-wide — assert that on a probe-reported **case-sensitive** location `LightStep` and `Lightstep` remain two distinct nodes compared exactly (no lowercasing or case-folding on read or write anywhere in the path), and that the case-insensitive fold never rewrites the existing node file to the incoming spelling of its own `@id`. Guards against T111's fold silently over-applying into a global canonical identity rule — the one change most likely to be mistaken for a simplification of it, and the one FR-028 forbids because identities double as wikilink targets for external readers whose link resolution this tool does not control (mirrors T095's scope-guard role for BUG-004) (depends on T111)

**Checkpoint**: BUG-008 resolved — a patch contributing a case-variant identity merges into the existing node under its recorded identity with a visible warning on a case-insensitive volume, creates a genuinely distinct node on a case-sensitive one, and never reports a well-formed file as malformed or names a file that does not exist; both behaviors are covered by tests that pass on either kind of disk
...
**Bugfix**: 2026-07-04 — BUG-005: Added new T097-T101 (no existing task reopened — `parsePatchBody`/`nodeFolder` correctly implemented what the spec asked for at the time; the spec itself had the gap). T097 adds a reserved-kind guard to `internal/core.ParsePatch`'s `parsePatchBody`, rejecting a patch body that declares a `kind: timeline` H1 section with `ErrPatchStructure` (spec.md FR-025/research.md D8b), so such a patch is refused wholesale, the same way any other malformed patch body already is (FR-002), instead of reaching `internal/app/graph/service.Apply`'s generic per-node path — where the missing `"timeline"` entry in `coreKindFolders` previously caused the node to be created at a bogus, pluralized `"timelines/<id>.md"` path that could collide with `applyTimeline`'s own, correctly-derived `timeline/yearly/`/`timeline/monthly/` period file for the same publication date. T098/T099 add the missing unit/E2E test coverage for this refusal; T100 tracks the already-applied `contracts/ast-contract.md` update; T101 is an optional, non-blocking defense-in-depth follow-up. Discovered via `arc lint`'s `uniqueBasename` check (specs/004-arc-lint), not by `arc apply` itself — see the "Bugfix: BUG-005" section above.

**Bugfix**: 2026-07-04 — BUG-006 (same-day correction of BUG-005's own fix): reported minutes after BUG-005 was marked resolved, against a real patch (`github.com/fogfish/bots/email-dev-thought.md`) that legitimately carries both a `# Source` and an explicit `# Timeline` section — BUG-005's "refuse the entire patch" resolution broke this real, valid usage. Reopened T097-T099 (T100/T101 superseded in place, see the "Bugfix: BUG-005" section above); added T102-T106. The corrected design (research.md D8b, revised): `ParsePatch` accepts a `kind: timeline` section like any other (T102 reverts T097's guard); `internal/app/graph/service.Apply`'s per-node loop now intercepts `Kind == "timeline"` before it ever reaches the generic node-creation path, folding its declared period into `applyTimeline`'s existing derivation (T102/T103) instead of writing it as a second, generic-`Node`-shaped file — which would have been wrong twice over, both the original folder-collision (BUG-005) and a second, previously-unnoticed format-incompatibility bug (`applyTimeline`'s period files use their own specialized rendering, never `core.RenderNode`'s generic one). A monthly-only declared period now also updates its yearly rollup (T103's cascade), closing the exact gap the user identified: "if patch contains only monthly node then yearly node in the graph has to be updated from the monthly node as well." Verified against the real, previously-failing patch and via new unit/E2E tests (T104/T105) and `go test ./...`. See the "Bugfix: BUG-006" section above.

**Bugfix**: 2026-07-04 — BUG-007: discovered via `arc lint`'s own node enumeration (specs/004-arc-lint), not by `arc apply` — a yearly timeline period file's `period: 2026` front-matter value decodes as a YAML integer, breaking `internal/core.ParseNode`'s `deriveNodeID` fallback for any generic reader (monthly files are unaffected). Added T107 (quote `period`'s value in `upsertTimelinePeriod`) and T108 (round-trip regression test). No task reopened — `upsertTimelinePeriod` correctly implemented research.md D8 as originally written; D8 itself never specified the string-safety requirement, now clarified via a new Bugfix paragraph and spec.md FR-010's clarification. See the "Bugfix: BUG-007" section above.

**Bugfix**: 2026-09-02 — BUG-008: a spec gap, reported against `arc apply` on macOS/APFS — FR-005/FR-006 never defined what "identity already exists" means when the storage location folds letter case, so `nodePath` + `store.Open` let the *filesystem* decide it: a patch contributing `Lightstep` to a graph already holding `LightStep` aborted the whole application with a diagnostic that reported a well-formed file as malformed and named a file that does not exist, while the same input on a case-sensitive volume silently forked one subject into two node files that `arc lint`'s duplicate-basename check cannot see either. Added T109-T117 (prerequisite `git ls-files` case-semantics check; the `fsys` letter-case probe; the canonical fold and its warning; the corrected basename diagnostic; both-behaviors tests; the 022 contract C7 extension; an FR-028 scope guard) plus spec.md FR-026/FR-027/FR-028/FR-029, SC-011/SC-012 and research.md D11. **No task reopened** — `validateNodeBasename`, `nodePath` and `readExistingNode` each correctly implemented what the spec asked for at the time; the spec itself had the gap. T109 blocks T111-T113: whether `git ls-files --error-unmatch` folds case under `core.ignorecase` is independent of the filesystem probe and must be established empirically, not assumed. See the "Bugfix: BUG-008" section above.

**Bugfix**: 2026-09-03 — 016-arc-revert BUG-002 — cross-reference, no task reopened here. `VCS.StageAll` (T057/T064) issues `git add -A` with no pathspec, which stages the **whole repository tree** rather than the graph directory, so for a graph nested inside a larger repository `arc apply` sweeps unrelated changes into its ingest commit — violating FR-011's "exactly one git commit containing every file the application touched." T064 is not reopened: it correctly implemented the contract as written, and the defect is in the shared adapter, not this feature's call site. Contract wording corrected in place; the adapter fix is specs/016-arc-revert/tasks.md T044.
