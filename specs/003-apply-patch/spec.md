...
- What happens when a "union" kind's already-populated scalar attribute or body text receives a differing incoming value (e.g. a recomputed derived score, or a reworded paraphrase of already-covered content)? This is not treated as a conflict needing review — the attribute keeps its first-written value (FR-023) and body text is reconciled paragraph-by-paragraph, silently absorbing near-duplicate paragraphs and appending only genuinely new ones (FR-024); this differs from FR-013's general "flag for review" rule, which still applies to `notes` and to `union, first-writer` kinds. *(Added — BUG-004.)*
- What happens when a patch's body declares a section for the format's own reserved index kind (`timeline`), e.g. as a self-describing period annotation alongside the document's own contribution? The tool MUST NOT refuse the patch and MUST NOT create a separate, ordinary node file for that section — it MUST fold the declared period into the tool's own timeline derivation (FR-010), so the graph never ends up with two different files describing the same period. If the declared period is a monthly one, the corresponding yearly period MUST be updated to match, the same as if the tool had derived it from the source's `published` date alone. *(Added — BUG-005, revised — BUG-006.)*
...
- **FR-004**: For every node section the patch carries, the tool MUST reconstruct the full node — its kind, identity, scalar attributes, and body content — from the patch's declared structure. Body content includes **every** relation the patch declares for that node, however many predicate-grouped blocks the patch's body organizes them into — none may be silently dropped, regardless of the specific body-formatting convention the patch uses to group them. *(Clarified — BUG-003.)*
...
- **FR-010**: The tool MUST derive the timeline period(s) — yearly and monthly — implied by the patch document's `published` date and MUST append an entry for the newly ingested document to each, in chronological order among that period's existing entries, creating the period's timeline file if it does not yet exist. Every field the tool writes into a period file's front matter MUST decode unambiguously with its intended type on a subsequent read by any generic graph-reading consumer (e.g. `period`'s value MUST remain a string even when it is a bare, numeric-looking year) — a period file is a node file like any other and MUST NOT be parseable only by this feature's own bespoke read path. *(Clarified — BUG-007.)*
...
- **FR-013**: When merging produces conflicting values for a field that is supposed to carry one authoritative value, the tool MUST preserve the first-written value as authoritative, MUST record the conflicting later value alongside it in a form that keeps the file human-readable and clearly marks the value as needing review, and MUST still complete the commit rather than aborting the whole application. *(Narrowed — BUG-004: FR-023/FR-024 carve out a "union" kind's scalar attributes and body text from this flagging rule; FR-013 continues to govern `notes` and any other kind's already-populated scalar fields.)*
...
- **FR-021**: The tool MUST offer an opt-in, more detailed progress mode that, for each node section it processes, identifies the node (by its title or identity) and states its outcome (created, merged, or merged-with-conflict); this detail MUST NOT appear in the tool's default output — FR-016's concise, on-success summary remains the default. *(Added — BUG-001.)*
- **FR-022**: When a domain/extension kind is registered with the "append" merge behavior (FR-019), applying a patch node of that kind to an already-existing node of the same identity MUST combine the existing node's relations and multi-valued fields with the patch's contribution the same way "union" does (FR-008) — duplicates collapsed, nothing dropped from either side — and MUST NOT flag any field as needing review, since "append" has no concept of a single authoritative value to diverge from. This is distinct from, and MUST NOT be confused with, the tool's own built-in timeline index (FR-010): that index is never itself patch-carried and uses a chronological, publication-date-derived insertion order that applies only to that one fixed index, not to domain/extension kinds that opt into "append." *(Added — BUG-002.)*
- **FR-023**: For a node kind whose merge behavior is "union" (FR-008), a scalar (non-list) attribute already set on the existing node MUST be preserved as first-written, and a later, differing incoming value for that same attribute MUST NOT be flagged as needing review under FR-013 — unlike FR-013's general rule, a kind registered for "union" merging is understood to carry attributes whose value may be legitimately recomputed or regenerated by its own producing pipeline on every contribution (e.g. a derived score), not a fact meant to be set once by one authoritative writer. *(Added — BUG-004.)*
- **FR-024**: For a node kind whose merge behavior is "union" (FR-008), the existing node's body text MUST be reconciled paragraph-by-paragraph rather than as one scalar value: each paragraph the incoming contribution supplies MUST be compared, using a deterministic (non-AI) text-similarity measure, against the existing node's paragraphs; an incoming paragraph judged sufficiently similar to one already present MUST be treated as already-present and dropped, and any other incoming paragraph MUST be appended to the existing body as new content. This reconciliation MUST NOT flag a conflict, for the same reason as FR-023. *(Added — BUG-004.)*
- **FR-025**: The tool MUST treat `timeline` as a format-reserved kind: a patch's body MAY carry a section for kind `timeline` (e.g. an extraction tool's own self-describing period annotation for the document it is contributing), but the tool MUST NOT reconstruct it as an ordinary node file. Instead, the tool MUST fold the section's declared period into its own timeline derivation (FR-010), exactly as if that period had been derived from the source's `published` date — including updating the corresponding yearly period when only a monthly one is declared (FR-010's existing yearly+monthly guarantee applies equally to a patch-declared period, not only to one derived from `published`). *(Added — BUG-005, revised — BUG-006: the original requirement wrongly required refusing the whole application; a real extraction pipeline legitimately carries this section.)*
...
- **SC-009**: 100% of the time a patch re-contributes to a "union" kind's already-existing node with a recomputed/regenerated scalar attribute or a reworded-but-substantively-the-same body paragraph, the application completes with zero conflict markers written and zero entries in `ApplyResult.Conflicts` for that node. *(Added — BUG-004.)*
- **SC-010**: 100% of patches whose body declares a section for the reserved `timeline` kind apply successfully, with the declared period folded into the tool's own timeline index (and its yearly rollup updated when only a monthly period is declared) — zero instances of a colliding or format-incompatible duplicate timeline file. *(Added — BUG-005, revised — BUG-006.)*
...
**Bugfix**: 2026-07-03 — BUG-001: Added FR-021 (opt-in detailed progress mode MUST identify each processed node and its outcome; MUST NOT appear in the default summary). The presentation-layer detail — which `bios.Reporter` calls carry this, exact label text, and gating on `--verbose`/`--quiet` — stays in `plan.md`/`research.md`/`data-model.md`/`contracts/` per this spec's technology-agnostic scope.

**Bugfix**: 2026-07-03 — BUG-002: Added FR-022. `append` (FR-019's fourth fixed merge behavior) previously had no requirement at all describing what it means for a domain/extension kind that is not the tool's own built-in timeline index — the tool crashed instead of merging. FR-022 closes that gap: for a domain-registered `append` kind, merging behaves like `union` (FR-008), with no conflict-flagging. The concrete algorithm (reusing `internal/core`'s existing edge/multi-valued-field union helpers) stays in `research.md` D6 per this spec's technology-agnostic scope.

**Bugfix**: 2026-07-03 — BUG-003: Clarified FR-004 — body content reconstruction MUST NOT silently drop any predicate-grouped block, however many a patch's body organizes, regardless of body-formatting convention. FR-004 itself was not wrong; the actual conflict was `research.md` D3's parser design recognizing only a `##`-heading form for predicate-grouped blocks, contradicting CORE §12.2's own "node bodies use bold labels, never headings" rule — this feature's own `quickstart.md` patch fixtures had independently drifted onto the same non-canonical `## Mentions` convention rather than the canonical `**Mentions**` bold-label form `github.com/fogfish/arcnet-spec`'s real example patches use. The parsing-convention fix and corrected fixtures stay in `research.md`/`quickstart.md`/`contracts/` per this spec's technology-agnostic scope.

**Bugfix**: 2026-07-03 — BUG-004: Added FR-023/FR-024 and SC-009, narrowed FR-013's scope. `MergeUnion`'s uniform "first-writer, flag any divergence" rule for scalar fields (research.md D6) assumed a genuine divergence is always a rare, real conflict between two independent writers. For an `entity` node, that assumption doesn't hold: derived scalar attributes (e.g. graph-analytics scores) are recomputed on every ingest run, and body text is wholesale LLM-resynthesized prose, not an incremental hand-edit — both diverge from the stored value on nearly every re-application, so `arc apply` flagged a conflict on almost every merge instead of the exceptional case FR-013 was written for. FR-023 makes a "union" kind's scalar attributes first-writer-wins with no flagging; FR-024 replaces whole-field text comparison with a paragraph-level, similarity-gated merge (near-duplicate paragraphs silently absorbed, genuinely new ones appended). The concrete algorithm (paragraph splitting, shingle/Jaccard similarity, threshold) stays in `research.md` D6 per this spec's technology-agnostic scope.

**Bugfix**: 2026-07-04 — BUG-005: Added FR-025 and SC-010, plus a new Edge Case bullet. CORE §12.2's "index kinds (`timeline`) are not carried" rule was already documented (research.md D8, FR-022's own aside) but never enforced — a patch declaring a `# Timeline` H1 section was accepted by `ParsePatch` and processed by `service.Apply`'s generic per-node path, whose folder-derivation helper falls back to pluralizing any unmapped kind name, turning the unmapped `"timeline"` kind into a bogus `"timelines/"` folder that can collide with the tool's own separately-derived `timeline/yearly/`/`timeline/monthly/` period file for the same publication date — discovered via `arc lint`'s `uniqueBasename` check (specs/004-arc-lint), not by `arc apply` itself. FR-025 promotes the already-documented invariant to an enforced requirement: the tool now refuses the entire application (FR-002's existing malformed-body refusal) when a patch declares `kind: timeline`. The guard's exact location (`internal/core.ParsePatch`) stays in `research.md` D8b per this spec's technology-agnostic scope.

**Bugfix**: 2026-07-04 — BUG-006 (same-day correction of BUG-005's own fix): BUG-005's "refuse the whole application" resolution was itself wrong — a real extraction pipeline was found to legitimately emit a `# Timeline` section alongside a document's own `# Source` section (a self-describing period annotation), and refusing such a patch broke real, valid usage. Revised FR-025/SC-010/the Edge Case bullet: the tool now accepts a patch-carried `timeline`-kind section, never reconstructs it as a generic node (it is folded into the tool's own FR-010 timeline derivation instead — see `research.md` D8b's revised decision for why this also avoids a second, previously-unnoticed format-incompatibility bug the first fix would have reintroduced), and — per the newly load-bearing part of FR-025 — a monthly-only declared period MUST also update the corresponding yearly period, so the two tiers never fall out of sync.

**Bugfix**: 2026-07-04 — BUG-007 (implementation drift, discovered via `arc lint`): a yearly timeline period file (`period: 2026`, a bare unquoted 4-digit scalar) failed to parse via `internal/core.ParseNode`, because YAML decodes it as an integer rather than a string, breaking `deriveNodeID`'s `period`-fallback type assertion — not a spec gap in FR-010 itself (the requirement to derive and append period entries was already fully correct), but the requirement never made explicit that a period file's front matter must remain string-decodable by any *generic* reader, which is exactly the gap `upsertTimelinePeriod`'s hand-built YAML (bypassing `internal/core`'s own safe encoder) fell into unnoticed, since `arc apply` itself never re-reads a period file through `core.ParseNode`. Clarified FR-010 with this expectation; see `research.md` D8's Bugfix paragraph for the concrete fix (`upsertTimelinePeriod` now writes `period` as an explicitly-quoted YAML string).
