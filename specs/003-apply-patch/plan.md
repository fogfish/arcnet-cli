...
**Storage**: The mounted graph root, accessed exclusively through the existing `internal/adapter/fsys` `Store`/`Mounter` — never raw `os.*` calls in domain/service code. *(Revised — BUG-008: this package is no longer unchanged by this feature. `Store`/`Mounter` gain the graph location's probed letter-case behavior, since `local.Open`/`local.Create` are where path-to-file resolution actually happens and are therefore the only place that can answer whether an identity already exists — research.md D11.)*
...
**Target Platform**: linux, darwin, windows (amd64 + arm64, `windows/arm64` excluded) — unchanged from `.goreleaser.yaml`. **Filesystem letter-case behavior matters here** (BUG-008, mirroring `specs/022-reference-type-folders/plan.md`'s own note): the developer machine is APFS (case-insensitive), CI is not — and it is a property of the *mounted volume holding the graph*, not of `GOOS`, so it is probed at run time, never inferred (research.md D11). Tests MUST cover both behaviors deterministically rather than inheriting whichever the test machine's disk provides.
...
**Constraints**: Target directory MUST already be an initialized graph (spec FR-014 — unlike `arc init`, `apply` never creates a graph root); no partial graph state may remain on any failure path (spec FR-015); a patch declaring an unrecognized node kind is applied using the safe `union` default and warns, never refused on this basis alone (spec FR-018, revised); `arc apply` itself remains fully local/offline (spec Assumptions); node identity existence is resolved against the graph location's probed letter-case behavior, folding a case variant onto the recorded identity with a visible warning where the location forces it, and never introducing a graph-wide canonical identity form (spec FR-026/FR-027/FR-028, research.md D11); `arc init`'s config-seed fetch MUST NOT block or fail initialization when the network is unavailable (`specs/002-arc-init/spec.md` FR-017)
...
**Bugfix**: 2026-07-04 — BUG-005: A patch declaring a `# Timeline` H1 section was silently accepted by `internal/core.ParsePatch` and processed by `internal/app/graph/service.Apply`'s generic per-node path — CORE §12.2's "index kinds (`timeline`) are not carried" invariant was already documented (research.md D8, spec.md FR-022's aside) but never enforced. The unmapped `"timeline"` kind fell through `nodeFolder`'s generic pluralization fallback into a bogus `"timelines/"` folder, producing a file that can collide with `applyTimeline`'s own, correctly-derived `timeline/yearly/`/`timeline/monthly/` period file for the same publication date — surfaced by `arc lint`'s `uniqueBasename` check (specs/004-arc-lint), not by `arc apply` itself. See `research.md` D8b for the new decision (a parse-time guard in `ParsePatch`'s `parsePatchBody`, returning `ErrPatchStructure`, consistent with FR-002's existing refusal model) and the new spec.md FR-025/SC-010/Edge Case bullet. No task reopened — `parsePatchBody`/`nodeFolder` correctly implemented what the spec asked for at the time; new tasks T097-T101 added (see `tasks.md`). **Superseded same-day — see BUG-006 immediately below.**

**Bugfix**: 2026-07-04 — BUG-006: BUG-005's own fix (refuse the whole application) was itself wrong — reported minutes later against a real patch (`github.com/fogfish/bots/email-dev-thought.md`) that legitimately carries a `# Timeline` section alongside its `# Source` section. See `research.md` D8b's revised decision: `ParsePatch` no longer refuses `kind: timeline`; `internal/app/graph/service.Apply`'s per-node loop instead intercepts it before the generic node-creation path (never writing it as a generic node file at all — which would also have been format-incompatible with `applyTimeline`'s own specialized period-file rendering, a second bug the first fix would have reintroduced) and folds its declared period into `applyTimeline`'s existing derivation, cascading a monthly-only declaration to also update its yearly rollup. Revised spec.md FR-025/SC-010/Edge Case bullet accordingly. Reopened tasks: T097-T099 (T100/T101 superseded in place); new tasks T102-T106 added (see `tasks.md`).

**Bugfix**: 2026-07-04 — BUG-007: Discovered via `arc lint`, not `arc apply` — a yearly timeline period file's `period: 2026` front-matter value (a bare, unquoted 4-digit scalar) decodes as a YAML integer, not a string, breaking `internal/core.ParseNode`'s `deriveNodeID` fallback for any *generic* reader of that file (monthly period files are unaffected, since `"2026-07"` isn't a valid bare YAML number). Root cause: `internal/app/graph/service/apply.go`'s `upsertTimelinePeriod` hand-builds front matter via raw string concatenation instead of `internal/core`'s own YAML-safe encoder, which every other node-writing path uses. See `research.md` D8's new Bugfix paragraph for the revised decision (quote `period`'s value explicitly) and the clarified spec.md FR-010. No task reopened — `upsertTimelinePeriod` correctly implemented research.md D8 as originally written; D8 itself never specified the string-safety requirement. New task added (see `tasks.md`).

**Bugfix**: 2026-09-02 — BUG-008: `arc apply` aborted an entire application on macOS/Windows when a patch contributed a node identity differing only in letter case from one already in the graph (`Lightstep` into a graph holding `LightStep`), with a diagnostic that reported a well-formed file as malformed and named a file that does not exist — while the same input on Linux silently forked one subject into two node files. Root cause: `nodePath` derives the target path purely from the incoming identity and `readExistingNode` hands that string to `store.Open`, so **the filesystem, not this codebase, decides whether an identity already exists** — an unstated per-volume dependency that makes FR-005 and FR-006 select opposite branches for identical input. See `research.md` D11 for the new decision (probe the graph root's letter-case behavior once per run, carry it with the mounted `Store`, fold a case variant onto the recorded identity with a warning where the location folds case, and correct the basename guard to compare against the name the store actually opened — via `ReadDir`, never `Stat`, whose `FileInfo.Name()` repeats the same lie one layer down) and the new spec.md FR-026/FR-027/FR-028/FR-029 and SC-011/SC-012. No task reopened — `validateNodeBasename`, `nodePath` and `readExistingNode` each correctly implemented what the spec asked for; the spec was silent. New tasks T109-T117 added (see `tasks.md`), of which T109 is a prerequisite empirical check on `git ls-files`' own independent `core.ignorecase` semantics and T117 guards FR-028's no-global-normalization constraint.
