...
**Bugfix**: 2026-09-03 — BUG-002 Updated from bugfix patch. Four of the git commands specified in contracts/vcs-port-contract.md's command mapping are scoped by git to the enclosing *repository*, not to the directory the subprocess runs in (verified against real git output), so they were correct only under a never-stated graph-root-equals-repository-root precondition. Every history operation must now be explicitly confined to the graph directory (spec.md FR-021); the contract's command mapping and its `ShowFile` row's incorrect `IsTracked` analogy are corrected accordingly.
...
**Constraints**: `service.Revert` MUST leave no partial state on any failure (FR-016) — mirrors `apply.go`'s existing bounded-rollback precedent (remove any newly-created scratch state, leave a partially-modified pre-existing file at its last fully-written state, recoverable via git). The per-node reconciliation path (research.md D7-D9) MUST NOT write to a node's `Attrs`/`Edges`/`HRefs` under any circumstance (FR-011) — this is a hard behavioral boundary, not a default. `internal/core` stays untouched and pure (no new function signatures) — this feature is additive at the `internal/app/graph`/`internal/adapter/git` layers only. Every `internal/adapter/git` command MUST be explicitly scoped to `dir`'s own subtree (FR-021) — setting the subprocess's working directory is *not* sufficient, since `git add -A`, `git log --grep`, `git diff-tree` and `git show <hash>:<path>` are each resolved by git against the enclosing repository rather than the working directory. Scope is therefore a per-command obligation of the adapter (pathspec, `--relative`, or a `./`-prefixed path, as recorded per row in contracts/vcs-port-contract.md), not a property inherited from how the graph happens to be laid out. *(Bugfix BUG-002, 2026-09-03)*
