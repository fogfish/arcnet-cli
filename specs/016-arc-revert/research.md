...
**Edge case**: zero matches → FR-002 refuse, "no ingest commit found for `<source-id>`."

~~More than one match is an integrity anomaly the format guarantees cannot happen (spec 003 FR-011: exactly one commit per successful apply) — treated as a hard refuse with a message pointing at `arc lint`'s `RuleIngestCommit` check, not a guess at which commit is "the" one.~~

**Corrected (Bugfix BUG-001, 2026-07-12)**: spec 003 FR-011's "exactly one ingest commit" invariant holds only *per apply invocation while that invocation's own contribution remains tracked* — it does not bound how many ingest commits a single `source-id` accumulates over the graph's whole history once `arc revert` exists. A user reverting an identifier, then re-applying the same document, then reverting it again is a normal, foreseeable sequence: `arc apply`'s own idempotency check (`IsTracked` on `sources/<id>.md`) only ever blocks a *second concurrent* apply while the first's source node is still present — once a revert removes it, a plain re-apply is free to create a brand-new ingest commit carrying the identical `Source-Id: <id>` trailer, since `buildCommitMessage` (`internal/app/graph/service/apply.go`) has no memory of any prior retraction. `CommitsMatching`'s `git log --all` ordering is already newest-first (matches `CommitsTouching`'s own documented convention) — treating `hashes[0]` as the ingest commit to act on is therefore always correct: every older match's own source node must, by construction, have already gone untracked (via a prior revert) before the newer ingest commit could exist at all, so there is no scenario where two matches are simultaneously "active." The genuinely-anomalous case this paragraph originally worried about (a hand-tampered or duplicated trailer) is `arc lint`'s `RuleIngestCommit` concern, not `arc revert`'s own — `arc revert` no longer refuses on match count alone (FR-020).
