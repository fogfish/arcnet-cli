...
|---|---|---|
| `CommitsMatching(dir, needle)` | `git log --all --fixed-strings --grep=<needle> --format=%H -- .` | ~~Already implemented (`internal/adapter/git/git.go:154`), reused verbatim — only the `graph` port interface is new, not the adapter body.~~ Corrected (Bugfix BUG-002, 2026-09-03): the trailing `-- .` pathspec is **required** and was missing. Without it `--all` searches the whole repository's history, so a graph nested in a larger repository matches ingest commits belonging to other graphs. "Reused verbatim" from 004-arc-lint is how the omission propagated here unexamined. |
| `ChangedPaths(dir, hash)` | `git diff-tree --no-commit-id --name-only -r --root --relative <hash>` | `--relative` is **required** (Bugfix BUG-002, 2026-09-03): without it `git diff-tree` emits paths relative to the *repository* root and includes every path the commit touched, so a caller treating them as graph-relative (D3's eligibility loop) either fails to resolve them or admits paths from outside the graph. `--relative` both rebases the output onto `dir` and filters out everything above it. Root-commit-safe: `git diff-tree` diffs a root commit against the empty tree automatically, unlike a plain two-dot `git diff`. |
...
| `ShowFile(dir, hash, path)` | `git show <hash>:./<path>` | The `./` prefix is **required** (Bugfix BUG-002, 2026-09-03): `git show <hash>:<path>` resolves `path` against the *repository* root, so in a nested graph every read fails with `fatal: path '<dir>/<path>' exists, but not '<path>'`. Returns the raw bytes of `path` as it existed at `hash`; a `path` that did not yet exist at `hash` is a normal, expected non-error case the caller (D8b) must handle by treating that commit as not having set the predicate yet, not by treating the git failure as fatal. Implemented by checking `git show`'s specific "does not exist" message markers, in the spirit of `IsTracked`'s own expected-vs-genuine failure distinction (`internal/adapter/git/git.go:128-144`). *(Verified — Bugfix BUG-002, 2026-09-03: the two markers `does not exist in` and `exists on disk, but not in` match exactly the two genuine absence conditions and do **not** match the path-resolution fatal above, which correctly falls through to `ErrGitShow`. An earlier draft of BUG-002 claimed otherwise; that claim was withdrawn. Note the analogy to `IsTracked` is loose — that method discriminates on exit code, this one on message text — so any future marker change must be re-verified against real `git show` output.)* |
...
4. **Every command is scoped to `dir`'s own subtree — explicitly, per command** *(Bugfix BUG-002, 2026-09-03)*. Setting the subprocess working directory to `dir` does **not** scope a git command: `git add -A` stages the whole tree, `git log --grep` searches the whole history, `git diff-tree` reports repository-root-relative paths, and `git show <hash>:<path>` resolves against the repository root. Each therefore carries its own explicit scoping mechanism — a `-- .` pathspec, `--relative`, or a `./`-prefixed path — as recorded per row in the mapping above. A caller may rely on every method reading, matching, and staging **only** within `dir`, whether `dir` is the repository root or a subdirectory of a larger repository (spec.md FR-021). `CommitsTouching` and `Blame` need no change: both already pass `path` after a `--` separator, which git resolves relative to the working directory.
5. **Test doubles**: `internal/app/graph/adapter/mock.VCS` (research.md D11) exposes one configurable return/error field per new method plus the existing `Calls` log, so `internal/app/graph/service/revert_test.go` can assert exactly which git primitives a given scenario invoked, the same way existing `apply_test.go` cases already assert on `Calls`.
