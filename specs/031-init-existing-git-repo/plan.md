...
**Bugfix**: 2026-09-04 — [BUG-001](bugs/BUG-001.md) Updated from bugfix patch. This plan's verification-first scope for FR-021-FR-025 (below) framed the nested-graph risk as **git-adapter scoping** — does the adapter stage, query and resolve paths against the right repository? That framing was correct and the adapter passed, but it does not reach the **filesystem-walk** layer, where the same never-stated "the graph root is exclusively `arc`'s" precondition also lived. FR-010 retired that precondition and nothing revisited the walk. Consequently `internal/app/graph` and `internal/app/lint` can no longer stay unmodified: `apply` must stop walking the tree entirely (spec.md FR-033), and `lint` must index foreign files the way `grep` already does (spec.md FR-035).

**Bugfix (round 2)**: 2026-09-04 — [BUG-001](bugs/BUG-001.md) `/speckit-bugfix-verify` found FR-034 (the read-path error sentinel) traceable to tasks but absent from this plan. It is now stated in Constraints below. No other plan content changed.

## Summary

`arc init` refuses to run inside an existing git repository, and gains `--skip-git-init` to add a graph to one instead. `cmd/arc/ctrl/init.go` registers the flag and resolves two environment facts through the git adapter before calling the service — the enclosing repository root (`git rev-parse --show-toplevel`) and whether the host's ignore rules exclude the target (`git check-ignore`) — passing both into `service.Init` as a new `kernel.InitOpts` value. `internal/adapter/git` gains four thin wrappers (`RepoRoot`, `IsIgnored`, `StagePaths`, `CommitPaths`); `internal/app/ctrl/port.VCS` swaps `StageAll`/`Commit` for `StagePaths`/`CommitPaths`. `internal/app/ctrl/service/init.go` is modified — unavoidably, see below — to add three guards, a mode branch around `vcs.Init`, a recorded write footprint replacing the static rollback list, and `.arc/.gitignore` replacing the root `.gitignore` write. ~~`internal/app/graph` and `internal/app/lint` are **not** touched.~~ *(superseded — Bugfix BUG-001, 2026-09-04: both are touched, at the walk/parse layer only. See the Constraints entry below.)*
...
*(Bugfix BUG-001, 2026-09-04)*: the verification-first method was sound; its **coverage** was not. "Nested" was implemented as one shape only — the graph in a subfolder, host content in a *sibling* directory — so the walk-layer defect was unreachable by construction and every scenario passed honestly. Parity has two independent axes and this plan conflated them:

| Axis | Meaning | Status |
|---|---|---|
| Repository nesting | graph root **below** the repository root | Verified (T048, [notes.md](notes.md)) |
| Root sharing | graph root **shared** with files `arc` did not write | Unverified — the defect |

The second axis is what FR-010 introduced; it is now covered by spec.md FR-032-FR-035.
...
**Performance Goals**: *(Bugfix BUG-001, 2026-09-04)* `arc apply` opens a number of graph files proportional to its patch's node count, never proportional to the graph root's file count — removing `guardNoOldFormatNodes` deletes a full-tree walk-and-parse from the front of every apply, which was both the correctness defect and an unbounded cost on a graph sharing its root with a large project (FR-033). For `arc init`: two additional `git` subprocess invocations (`rev-parse --show-toplevel`, and `check-ignore` only when a parent repository was found). Negligible against the four-to-five invocations init already makes; well inside Constitution X's ~100ms first-output budget

**Constraints**: guards MUST all precede `resolveLocalRoot` so a refusal never touches disk (FR-005) and so detection works for a not-yet-existing target (research D2) — this reorders the current sequence; `CommitPaths` MUST be a new method rather than a signature change to `Commit`, because the single concrete `git.VCS` type structurally satisfies both `ctrl` and `graph` port interfaces (ADR 001) and `graph`'s declares the pathspec-free shape; ~~`internal/app/graph` and `internal/app/lint` MUST remain unmodified~~ *(superseded — Bugfix BUG-001, 2026-09-04: this constraint held only while the graph root was exclusively `arc`'s. FR-032-FR-035 require `internal/app/graph/service/apply.go` to drop its whole-graph guard and `internal/app/lint/service/lint.go` to index foreign files. Both changes stay inside the walk/parse layer; no port, adapter or command contract changes.)* `apply`'s read path MUST also distinguish the operation it is reporting: a rejection raised while *reading* a targeted node file carries a read-path sentinel, never `ErrNodeWrite` (spec.md FR-034). Borrowing the write sentinel for a read is what turned this bug's symptom into a message describing an operation the code never attempts, and Constitution X's actionable-output rule (spec.md FR-029) is not satisfied by an accurate error attached to the wrong verb. *(Bugfix BUG-001, 2026-09-04)*
