...
- **FR-020**: Schema-index loading (`internal/app/schema/service.decodeTypeDef`, consulted by `Resolve` and therefore by `arc apply`, `arc lint`, and `arc apply schema` alike) MUST NOT require a `Class` document's whole-node `merge` field to be present or valid — an absent or unrecognized value MUST be treated as "no whole-node merge declared" (consistent with this field having no effect on reconciliation since FR-015), not a validation failure. A `Property` document's `merge` field remains mandatory and fully validated exactly as before (FR-002/FR-013) — predicate-level merge is still the sole authority `arc apply` consults, so this requirement narrows validation strictness for `Class` documents only. *(Bugfix 018/BUG-001, 2026-07-19)*
...
- **SC-008** *(018/BUG-001)*: Importing a well-formed, CORE-conformant `Class` definition with no `merge` field — via `arc apply schema`, or as hand-authored `_schema/types/` content read by `arc apply`/`arc lint` — succeeds in 100% of tested cases, creating/loading the type definition rather than failing the whole operation.
...
- The whole-node `merge` field on a type's schema document (introduced as a temporary bridge so today's whole-node dispatch could keep working) is no longer consulted by arc apply once this feature ships. ~~Removing that field from the schema-document shape itself is out of scope; it simply becomes unused by reconciliation.~~ Superseded by FR-020 (Bugfix 018/BUG-001, 2026-07-19): the field's *presence* is no longer validated as mandatory either — a `Class` document's own schema-index loading (`decodeTypeDef`) no longer requires a `merge` value, though a built-in/auto-registered `Class` document continues to be seeded with `merge: union` for shape continuity. A `Property` document's `merge` field remains mandatory and fully validated (FR-002/FR-013).
...
**Bugfix**: 2026-07-19 — 018/BUG-001 added FR-020 and SC-008, and updated the whole-node-`merge`-field assumption: `arc apply schema` was rejecting a well-formed, published extension's `Class` definition (e.g. a `Hypothesis` type with no `merge` field) because `internal/app/schema/service.decodeTypeDef` still treated the field as mandatory — a validation rule this feature's own FR-015 (merge no longer consulted for reconciliation) should have relaxed at the time but explicitly scoped out. `Property`-level `merge` validation is unaffected; it remains mandatory since predicate-level merge is still load-bearing.
