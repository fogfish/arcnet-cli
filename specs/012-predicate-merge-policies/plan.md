...
**Bugfix**: 2026-07-19 — 018/BUG-001 Updated from bugfix patch. `internal/app/schema/service/schema.go`'s `decodeTypeDef` (already in this plan's Project Structure as "validMergeOps: 5 → 7 values") is back in scope: it must stop requiring a `Class` document's `merge` field to be present/valid (FR-020), while `decodePredicateDef`'s `Property`-level check is unchanged. No Constitution Check section changes — this narrows validation strictness for `Class` documents only, still inside `internal/app/schema/service`, no new port/adapter/command.
