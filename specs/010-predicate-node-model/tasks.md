...
- [X] T024 (reopened — BUG-007, reclosed by T117-T119) Update `walkNodeBody` in `internal/core/markdown.go`: keep its existing leading-prose/optional-bare-list/heading-or-bold-label-blocks/trailing-prose structural recognition, but (a) route leading/trailing prose into `Texts[textPredicateFor(...)]` instead of `Text`/`Notes`, and (b) flatten the bare list's items and every heading/bold-label block's items into one `Edges` slice in document order, with no per-block grouping key retained (depends on T016, T017, T023). Correctly implemented the two-slot (leading/trailing) shape this task was scoped to build, migrating off the old fixed `Text`/`Notes` fields — appropriate at the time (research.md D4 explicitly flagged it as a temporary stopgap, before spec 011/013 existed). Reopened because that stopgap shape was never revisited once spec 011/013 gave the parser real schema-role knowledge: `textPredicateFor(nodeType, false)`'s trailing key has no counterpart in ARCNET-CORE's actual model (one default `text`-role predicate per type, not two). Closed by T117-T119 (FR-026).
...
- [X] T089 (reopened — BUG-005, reclosed by T108) [P] Add regression tests: a labeled `text`-role block (plain prose or non-wikilink list) round-trips through `arc apply` intact and merges per its predicate's declared behavior on re-apply; a labeled `edge`/`link`-role block is unaffected; an unregistered label with non-wikilink content auto-registers as `role: text, merge: append` and its content survives; a mixed list (some wikilink lines, some not) preserves both the extracted edges and the remaining text (`internal/core/markdown_test.go`, `internal/app/graph/service/apply_test.go`, `cmd/arc/graph/apply_test.go`). Correctly implemented and tested the non-wikilink-*list* sub-case; reopened because the "plain prose" sub-case this task's own description names was never actually tested — `classifyNodeBody` cannot produce a titled block from a title followed by a paragraph at all (only a title followed by a list), so no test in any of the three named files exercises it. Closed by T108 (FR-024).
...
- [X] T097 (reopened — BUG-006, reclosed by T113) Render a heading/bold-label for a `role: text` predicate's `Texts` entry on write, mirroring `renderEdges`'s existing `role: link` heading logic (`## Label` for `RenderNode`, `**Label**` for `RenderPatch`) in `renderNodeBody`'s "other Texts keys" loop (`internal/core/markdown.go`) (FR-021, depends on T095). **This description was wrong, not merely incomplete** — ARCNET-CORE §5 gives text-role predicates a *different* rule than link-role predicates: always bold, in every format, never a caller-dependent heading. The implementation as described produces `## Label` in `RenderNode` output; BUG-006 reports this as wrong. Closed by T113 (FR-025).
...
**Bugfix**: 2026-09-05 — BUG-005 Updated from bugfix patch: reopened T089 with a correction note (correctly implemented and tested its non-wikilink-list sub-case; the "plain prose" sub-case it also named was never implemented or tested — see note above) and added Phase 9 (T106-T111): `classifyNodeBody`'s title-pairing check only ever matches a title followed by a list, never a title followed by a prose paragraph, so a `**Label**`\nparagraph block (a `text`-role predicate's natural body shape) is silently absorbed into the type-blind trailing slot instead of resolving to its own predicate (FR-024).

---

## Phase 9: Bugfix BUG-005 — Title Followed by a Prose Paragraph Never Pairs, Falls Into the Type-Blind Trailing Slot

**Purpose**: Addresses [bugs/BUG-005.md](bugs/BUG-005.md): a `**Label**`/`## Label` title immediately followed by a plain prose paragraph (not a list) — ARCNET-CORE §5's own `text`-role body shape, e.g. `**Overview**` followed by a paragraph of prose — is never recognized as a titled block at all. `classifyNodeBody`'s pairing check (`internal/core/markdown.go:761-765`) only tests whether the sibling immediately after a title is an `*ast.List`; a `*ast.Paragraph` sibling fails that check, so both the title and the following prose fall through individually as plain, untitled `bodyBlockProse` and get swept into `walkNodeBody`'s leading/trailing structural slots — landing, in the common case, under the type-blind trailing key (`textPredicateFor(type, false)`, e.g. `notes`) with the literal `**Label**` text embedded inside the prose value. This corrupts both which predicate the content is captured under (`arc lint`'s `typeOptional` rule then rejects `notes` for `Entity`/`Resource`, which CORE 0.12 retired it from — spec 023 FR-033) and where the block renders relative to edges (the reserved trailing slot renders after every edge/link block; any genuinely named text predicate already renders before them, per `renderNodeBody`'s existing "other Texts keys" loop). Root cause and fix approach: extend `classifyNodeBody`'s title-pairing to accept a following `*ast.Paragraph` as a valid pairing target alongside the existing `*ast.List` case, and route the resulting `bodyBlockTitled`-with-prose variant through the same `resolveLabelPredicate`/auto-registration path `walkNodeBody` already applies to the list variant.

- [X] T106 [P] Extend `classifyNodeBody` to pair a title (`blockTitle`) with immediately following prose, not only a list. **Corrected mid-implementation** (found before any code was written to be more involved than described): a title occupying its own paragraph, followed by a *separate* Paragraph sibling (blank line in between) is one shape — but CommonMark's actual behavior for the bug's own literal reproduction (`**Overview**\nOverview text`, no blank line) is to join both lines into a *single* goldmark Paragraph node via a soft line break, so there is no second AST sibling for any title+sibling pairing check to ever find. Implemented as: (a) `splitLeadingBoldLabel` (`internal/core/markdown.go`, next to `boldLabel`) recognizes a paragraph whose *first raw source line* alone is a complete `**Label**` run, splitting it into the label and the paragraph's remaining lines; (b) `collectTitledProse` (`internal/core/markdown.go`) absorbs any further plain (non-title) Paragraph siblings as continuation content, stopping at a child that is itself title-shaped (via either `blockTitle` or `splitLeadingBoldLabel`, so a later title's own same-paragraph-joined content is never misread as the earlier title's continuation — verified against an adjacent-titles-with-no-content-between regression); `bodyBlock` gained one `content []string` field (replacing the single `list`-only prior shape for the titled-prose case) (FR-024)
- [X] T107 Extend `walkNodeBody`'s `bodyBlockTitled` handling to resolve `b.content` (T106) through `resolveLabelPredicate` exactly as the list variant already does: a resolved-or-unresolved paragraph block is always captured as prose under the resolved (or label-derived, auto-registered) predicate name — there is no wikilink-list vs. prose distinction to make for a paragraph the way the list variant makes for `role != "text"`, since a paragraph cannot carry a role:edge/link's targets. The joined content is routed through `extractInlineLinks`/`reconstructHRefs` (a new `namedTextParas` map + final-loop branch, alongside the pre-existing list-sourced `namedTexts`/`renderTextListLines` path) rather than `listItemLines`/`renderTextListLines`, since CORE §5's `href` role — not FR-020's list-item verbatim-preservation rule — governs genuine prose (FR-024, depends on T106)
- [X] T108 [P] Add regression tests, reclosing T089 (`internal/core/markdown_test.go`): the bug's exact reproduction (same-paragraph title+prose) resolves to its own predicate, is absent from `Texts["notes"]`, and renders before the flat edge list and before the `## Concerns` link-role block (`TestParsePatchLabeledProseBlockResolvesToOwnPredicate`); multi-paragraph content under one label joins correctly (`TestParsePatchLabeledMultiParagraphProseBlockRoundTrips`); an unresolved label auto-registers `role: text`, recovers its literal label (FR-021), and an inline wikilink in its prose extracts to `HRefs` (`TestParsePatchUnresolvedLabeledProseBlockAutoRegistersAsText`); round-trip idempotency for the same-paragraph shape (`TestRoundTripLabeledProseBlockIdempotent`); and the adjacent-titles-with-no-content-between edge case T106's fix required (`TestParsePatchAdjacentTitlesNoContentBetween`) (depends on T106, T107)
- [X] T109 [P] Add lint-facing regression tests (`internal/app/lint/service/rules_type_conformance_test.go`): the bug's exact reproduction, parsed and fed through `checkTypeOptional` against an index recording `overview` as `Entity`'s Optional, produces zero violations (`TestCheckTypeOptionalBug005LabeledProseBlockPasses`); and — confirming this fix does **not** silently close spec 023 BUG-001's own still-open T068 deviation — a node with genuinely unlabeled trailing prose under the literal `notes` key on `Entity` still correctly fails `typeOptional` with the exact message this bug was originally reported against (`TestCheckTypeOptionalUnlabeledTrailingProseOnEntityStillFails`) (depends on T107)
- [X] T110 Ran `go build ./... && go test ./... && go vet ./...`; all green. `staticcheck ./...` could not run in this environment (`internal error in importing "internal/byteorder" ...: export data version 4 is greater than maximum supported version 2`) — a pre-existing local toolchain/staticcheck-version mismatch (`go version go1.27.0`, `staticcheck 2026.1`/`v0.7.0`) unrelated to this change; `go vet` covers the same module cleanly (depends on T106-T109)
- [X] T111 Manually re-verified via a real `arc init`/`arc apply`/`arc lint` run against this bug's exact reported reproduction (an `Entity` node with `*important text*`, a `derivedFrom` edge, a `**Overview**\nOverview text` block sharing one paragraph with no blank line, and a `**Concerns**` link-role block): the written node file shows `## Overview` rendered immediately after the leading text and before the flat `derivedFrom` bullet and `## Concerns`, exactly matching the required predicate-role ordering; `arc lint` no longer reports `overview` under `[typeOptional]` once `Entity`'s schema records it as Optional — the same one-time maintainer step any newly auto-registered predicate already requires, confirmed symmetric by `concerns`/`derivedFrom` (auto-registered via the pre-existing BUG-002/003 edge/link path) needing and receiving the identical step in the same run (depends on T110)

**Checkpoint**: BUG-005 fixed — a `**Label**`-headed body block resolves against the schema whether it is followed by a list or by a prose paragraph; a text-role paragraph block is captured under its own predicate name and renders in the same body position as any other named text-role predicate, no longer absorbed into the type-blind leading/trailing structural slots.

**Bugfix**: 2026-09-05 — BUG-006 Updated from bugfix patch: reopened T097 with a correction note (the task's own description mirrored the wrong rule) and added Phase 10 (T112-T116): a named text-role predicate's block renders with a `## Label` heading in `RenderNode` output instead of the `**Label**` bold label ARCNET-CORE §5 requires in every format — only link-role predicate groups vary their markup by caller (FR-025).

---

## Phase 10: Bugfix BUG-006 — Text-Role Predicate Block Renders With a Heading Instead of the Required Bold Label

**Purpose**: Addresses [bugs/BUG-006.md](bugs/BUG-006.md): `renderNodeBody`'s "other Texts keys" loop (`internal/core/markdown.go:1419-1421`, `writeText(k, true)`) renders a named text-role predicate's block using the *same* `if patchFormat { bold } else { heading }` branch `renderEdges` correctly uses for link-role predicate groups — producing `## Label` in `RenderNode` output. ARCNET-CORE §5 states the opposite for text-role predicates: "continue into other `text`-role predicates but prepend, each block with bold, capital predicate name or its label" — no caller-format variation, unlike the neighboring link-role rule T097 (BUG-003, 2026-07-21) mistakenly copied onto it. Root cause and fix: `writeText`'s "other Texts keys" call site must always emit `"**" + label + "**\n"`, independent of `patchFormat`; `renderEdges`'s own, separate link-role format-by-caller logic is untouched.

- [X] T112 [P] Fix `writeText`'s "other Texts keys" call site (`internal/core/markdown.go:1419-1421`) to always render `"**" + label + "**\n"`, removing the `if patchFormat` branch for this call — `renderEdges`'s independent link-role heading/bold-by-caller logic (`internal/core/markdown.go:1495` onward) is unaffected and must keep its existing, correct behavior (FR-025)
- [X] T113 Correct the stale comment at `internal/core/markdown.go:1406-1410` ("... using the same label-resolution and heading-vs-bold-label format rules already defined for a role: link edge group") to state the actual (always-bold) rule, and update BUG-005's own `TestParsePatchLabeledProseBlockResolvesToOwnPredicate` (`internal/core/markdown_test.go`) — added the same day this bug was found — to assert `**Overview**\nOverview text`, not the now-corrected-away `## Overview\nOverview text` (FR-025, depends on T112)
- [X] T114 [P] Add a regression test asserting a text-role predicate's block renders bold in **both** `RenderNode` and `RenderPatch` output, alongside an existing link-role predicate in the same node still correctly varying its own markup by caller — locking in the two rules' independence, not just the text-role half (`internal/core/markdown_test.go`) (depends on T112)
- [X] T115 Audit `TestParsePatchLabeledTextRoleListPreservesWikilinksAndListShapeVerbatim`, `TestRenderNodeSeparatelyLabeledEdgeBlocksStayDistinctGroups`, and any other existing test rendering a text-role predicate's block in `internal/core/markdown_test.go`, `internal/app/graph/service/apply_test.go`, and `cmd/arc/graph/apply_test.go` — confirm none assert the now-incorrect `##`-heading shape for a genuinely text-role (not link-role) predicate; fix any that do (depends on T112). Found and fixed one: `cmd/arc/graph/apply_test.go`'s `TestApplyLabeledBlockShapeSurvivesWikilinksListMarkersHeadingsAndGrouping` asserted `## Assumptions` for `assumptions`, a genuinely text-role predicate (confirmed by its own adjacent schema assertion, `role: text`) — corrected to `**Assumptions**`. `## Related Aporias` in the same test is correctly unchanged (role: link).
- [X] T116 Ran `go build ./... && go test ./... && go vet ./...`; all green (`staticcheck` unavailable — pre-existing local toolchain mismatch, see T110). Manually re-verified against BUG-005's own reproduction (`arc init`/`arc apply`): the written node file shows `**Overview**\nOverview text`, not `## Overview`, while the link-role `## Concerns` block is unaffected (depends on T112-T115)

**Checkpoint**: BUG-006 fixed — a named text-role predicate's block always renders with a bold label, in both `RenderNode` and `RenderPatch` output; only link-role predicate groups still vary their markup by caller.

**Bugfix**: 2026-09-05 — BUG-007 Updated from bugfix patch: reopened T024 with a correction note (the two-slot shape it built was appropriate at the time, superseded once spec 011/013 gave the parser real schema-role knowledge) and added Phase 11 (T117-T121): `textPredicateFor`'s hardcoded trailing-slot key has no counterpart in ARCNET-CORE's model — collapsing it into the type's single default `text`-role predicate (FR-026). Companion to spec 023's own BUG-002, which retires the vocabulary name (`notes`/`relevance`) this slot rendered under; the two land together in the same implementation pass.

---

## Phase 11: Bugfix BUG-007 — The Trailing-Slot Structural Split Has No Counterpart in ARCNET-CORE's Model

**Purpose**: Addresses [bugs/BUG-007.md](bugs/BUG-007.md): `textPredicateFor(nodeType, leading bool)` returns a different, type-blind predicate name for "trailing" prose than for "leading" prose, and `walkNodeBody`/`renderNodeBody` store/render them as two structurally separate `Texts` keys. ARCNET-CORE §5 has no such second slot — exactly one default (unlabeled) `text`-role predicate per type, rendered first, plus any number of explicitly *named* ones. This is spec 010's own pre-schema-role stopgap (research.md D4), never revisited once spec 011/013 gave the parser real schema knowledge. Root cause and fix: collapse `textPredicateFor` to one key per type; merge `walkNodeBody`'s trailing-scan accumulation into the *same* key as leading (blank-line-joined, document order); `renderNodeBody` writes the merged value once, in the existing leading position. Land together with spec 023's own [BUG-002](../../023-core-vocabulary-conformance/bugs/BUG-002.md), which retires the `notes`/`relevance` vocabulary names this slot rendered under.

- [X] T117 [P] Collapse `textPredicateFor`'s signature from `func textPredicateFor(nodeType string, leading bool) string` to `func textPredicateFor(nodeType string) string` — one key per type (`Source`→`abstract`, everything else→`text`, per spec 023 BUG-002's own retirement of `Reference`'s `relevance`) — in `internal/core/markdown.go`; updated `TextPredicateFor`'s exported wrapper and every call site (`internal/app/graph/service/apply.go`'s `distinctPredicates`, `internal/app/graph/service/revert.go`'s duplicate) (FR-026)
- [X] T118 In `walkNodeBody` (`internal/core/markdown.go`), merge the trailing scan's accumulated raw text into the *same* `texts[textPredicateFor(nodeType)]` value as the leading scan's (blank-line-joined) instead of a second, differently-keyed write — the positional leading-vs-trailing *scanning* logic is unchanged, only the destination key merges (FR-026, depends on T117)
- [X] T119 In `renderNodeBody` (`internal/core/markdown.go`), removed the separate trailing-key write; the single merged primary-key value renders once, in the existing leading position. Updated `revertLeadingKey`'s duplicate copy and its two call-site helpers, `physicalTextKeyOrder`/`mapTextParagraphs` (`internal/app/graph/service/revert.go`), dropping their now-meaningless `trailingKey` parameter and the module-level `revertTrailingKey = "notes"` constant entirely (FR-026, depends on T117, T118)
- [X] T120 [P] Added regression tests (`internal/core/markdown_test.go`): `TestParsePatchLeadingAndTrailingProseMergeIntoOneKey` and `TestRoundTripLeadingAndTrailingProseIdempotent` cover the merge and its idempotency directly. Audit found and fixed a **real, non-cosmetic regression the merge itself introduced**, not just stale assertions: merging leading and trailing prose into one physical render position can put two *separate* untitled bare lists string-adjacent, separated only by a blank line — CommonMark merges same-marker bullet lists across a blank line into one loose List node (verified directly against goldmark's AST), silently losing the boundary between them on the next parse and breaking FR-015's idempotent round-trip (`TestRoundTripMultipleUntitledListsIdempotent` caught this). Fixed by alternating the bullet marker ("*"/"+") every time `nextBareList` renders an untitled list within one node's body, so two such lists always start distinct List nodes regardless of blank-line adjacency; `renderBareTextListLines` (now dead) was deleted in favor of calling `renderMarkedListLines` directly through `nextBareList`. Also updated `TestParsePatchMultipleUntitledListsPreserveBulletShapeAndTitlePairing` (asserts against the merged `"abstract"` key, second list now `"+"`-marked) and `TestParseRenderRoundTripPreservesLeadingProseKey` (`Reference` fixture now expects `"text"`, not `"relevance"`) (depends on T117-T119)
- [X] T121 Ran `go build ./... && go test ./... && go vet ./...`; all green (`staticcheck` unavailable, see T110/T116). Manually re-verified via a real `arc init`/`arc apply`/`arc lint` run: an `Entity` node with unlabeled prose both before and after a `derivedFrom` edge merges both spans into one value rendered once at the top; a `Reference` node's prose renders as plain "text", not "relevance"; `arc lint` and a full-graph `grep` confirm neither `notes` nor `relevance` appears anywhere in the written graph (depends on T117-T120)

**Checkpoint**: BUG-007 fixed — `textPredicateFor` returns one key per type; unlabeled leading and trailing prose merge into that single value, rendered once in the leading position; no structural trailing-slot concept remains anywhere in the parser or renderer.
