...
- [X] T086 [P] A bare list with no preceding label keeps today's per-line classification (explicit `predicate::` tag or wikilink shape); a non-matching line now falls through to text instead of being dropped, in `collectListLinks`/`walkNodeBody` (`internal/core/markdown.go`) (FR-019, Edge Cases)
