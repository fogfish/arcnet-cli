...
- **Q: Which predicate lists govern `Reference`? Upstream §11.6's normative `Class` block and its own 0.9→0.10 revision note disagree.** → **A: The revision note's lists.** `Reference` requires `title`, `ref`, `relevance` and offers `url`, `authors`, `year`, `doi`, `status`, `isCitedBy`, `notes`. This matches the description's "owning exactly those displaced predicates" and is the only reading under which §11.6's own worked example — which carries `ref`, `status`, and leading prose — conforms to the type it illustrates.
  ...
  > **SUPERSEDED 2026-08-23 by [`023-core-vocabulary-conformance`](../023-core-vocabulary-conformance/spec.md) (FR-028).** The revision note this answer rests on is **gone from ARCNET-CORE v0.11**, whose normative §11.6 `Class` block requires **`title` alone**. Spec 023 adopts v0.11: `Reference` requires `title`, and `ref`, `relevance`, `status`, and `notes` are retained as **Optional** — which preserves both halves of this answer's reasoning (§11.6's worked example still conforms, and 022's body-prose keying of a `Reference`'s leading prose to `relevance` is untouched) while dropping two requirements the current specification does not make. The change is a pure relaxation, so no graph that lint‑ed clean under this clarification can start failing. Full reasoning: [plan.md F3](../023-core-vocabulary-conformance/plan.md), [research.md D3](../023-core-vocabulary-conformance/research.md).
  >
  > This answer was correct for the document that existed when it was made; it is recorded as superseded rather than rewritten, so the decision trail stays readable.
