...
**Bugfix**: 2026-09-03 — 016-arc-revert BUG-002 — cross-reference, no task reopened here. `VCS.StageAll`, specified by this feature's contracts/vcs-port-contract.md as "`git add -A` with working directory `dir`", is not scoped by that working directory: `git add -A` with no pathspec stages the whole repository tree. Harmless for `arc init` itself (it creates the repository it stages into, so the two roots always coincide), but the same shared adapter method is reused by `arc apply`/`arc revert`, where they need not. The contract wording is corrected in place; the adapter fix is specs/016-arc-revert/tasks.md T044.
