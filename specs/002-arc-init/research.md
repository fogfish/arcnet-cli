...
## D8: Config-seed fetch (cross-referenced from `specs/003-apply-patch`, 2026-07-02)

`specs/003-apply-patch` adds FR-017 to this spec: `arc init` now also seeds `.arc/config.yml` with per-kind merge-rule defaults, preferring a live fetch of `github.com/fogfish/arcnet-spec`'s canonical config over a built-in, core-kinds-only fallback used whenever the fetch fails for any reason (never a hard `arc init` failure). The full design — `internal/app/config.Default`'s fetch/fallback contract, the promoted `internal/app/ctrl.Init` signature gaining a `configSeed []byte` parameter, the new `--verbose` Reporter step, and the fixed-timeout/no-override tension — is owned by `specs/003-apply-patch/research.md` D5 (revised), not restated here, since that feature is what introduces the first real consumer of this configuration and therefore owns its design. This note exists only so a reader of this spec's own research log is pointed at where the behavior it now documents (FR-017) is actually decided.
