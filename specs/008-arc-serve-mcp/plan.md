...
**(Added — BUG-001)** `github.com/fogfish/logger/v3` (new — a `log/slog` handler package; `logger.New(logger.Console...)` returns a colored, single-line-per-record `*slog.Logger` for terminal output) backs `logCall`'s rewrite to satisfy spec FR-019 (revised)/FR-020/FR-021 — the server's second third-party dependency, after `github.com/modelcontextprotocol/go-sdk`.
...
| XIV — Versioning/Security | Yes | PASS — no existing `--json`/`--plain` contract touched (this command has neither); ~~adds exactly one new third-party dependency (`github.com/modelcontextprotocol/go-sdk`) for `govulncheck` to track going forward — a new, deliberate, and singular supply-chain addition, not a speculative one~~ **(revised — BUG-001)**: adds a second new third-party dependency, `github.com/fogfish/logger/v3` (structured/colored logging, spec FR-021), for `govulncheck` to track alongside `github.com/modelcontextprotocol/go-sdk` — both deliberate, neither speculative |

**Bugfix**: 2026-08-29 — BUG-001 Technical Context/Constraints annotated with the new `github.com/fogfish/logger/v3` dependency and the revised per-call log-line shape (filter summary + result-size count, spec FR-019 revised/FR-020/FR-021); Constitution Check row XIV updated for the second dependency.
