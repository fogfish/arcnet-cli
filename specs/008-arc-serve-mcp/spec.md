...
- **FR-019**: ~~For every tool call, the server MUST emit one line to stderr recording the tool name, its key arguments (id, pattern, or depth as applicable), and whether it succeeded or failed — sufficient for an operator to reconstruct what an agent asked for and what it got back, without a separate metrics or tracing system.~~ *(superseded — BUG-001: the parenthetical predates the `filter` argument existing on any tool — added later by 027-triple-filter-model/028-node-match-filter to `subgraph_get`/`context_retrieve`/`node_match` — and "what it got back" was never actually captured. See the revised FR-019 and new FR-020/FR-021 below.)*
- **FR-019** (revised — BUG-001): For every tool call, the server MUST emit one log line recording the tool name, its key arguments — including a compact summary of its `filter` argument when the tool accepts one, not merely whether one was supplied — and whether it succeeded or failed, sufficient for an operator to reconstruct what an agent asked for, without a separate metrics or tracing system.
- **FR-020** (new — BUG-001): For every successful tool call, the server MUST additionally record the size of what it returned (e.g. a match, node, or fact count; for `schema`, its predicate and class counts), sufficient for an operator to reconstruct what an agent got back, without a separate metrics or tracing system.
- **FR-021** (new — BUG-001): The server's per-call log line MUST be emitted via structured logging, with colored output when writing to a terminal, and MUST remain a single line per call regardless of how many arguments or result fields it records.

**Bugfix**: 2026-08-29 — BUG-001 FR-019 amended (its "key arguments" parenthetical no longer names an exhaustive, stale argument list and now explicitly covers the `filter` argument) and split; FR-020 (result-size logging) and FR-021 (structured, colored, single-line output) added to close the gap between this requirement's stated goal and the tool surface that grew around it.
...
- **SC-008**: ~~Given the server's stderr output from a session, an operator can identify which tool was called, its key arguments, and whether it succeeded, for 100% of calls made in that session, without any additional tooling.~~ *(superseded — BUG-001: silent on the filter argument and on result size; see revised SC-008 and new SC-009.)*
- **SC-008** (revised — BUG-001): Given the server's log output from a session, an operator can identify which tool was called, its key arguments — including the actual `filter` content whenever one was supplied, not just whether one was present — and whether it succeeded, for 100% of calls made in that session, without any additional tooling.
- **SC-009** (new — BUG-001): For 100% of successful tool calls in a session, the server's log output lets an operator determine the size of what was returned (a count of matches, nodes, or facts, as applicable to that tool) without a separate metrics or tracing system.

**Bugfix**: 2026-08-29 — BUG-001 SC-008 revised to require the filter's actual content (not a presence flag); SC-009 added for result-size observability.
...
- **(Added — BUG-001)** FR-021's "structured logging, with colored output when writing to a terminal" is satisfied by adopting `log/slog` plus `github.com/fogfish/logger`'s colored console handler as this feature's logging technology — a new third-party dependency, not present when this spec was originally written; a filter's logged summary is illustrative/diagnostic only (e.g. a compact rendering of its statement count or JSON), not a guarantee of exact reproducibility of the original call.
