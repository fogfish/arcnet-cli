...
│   │                         #   reuses grep.go's optsFilter plus its own --predicate flag
│   │                         #   (traversal-scoping only, specs/027-triple-filter-model
...
│                             #   Filter{Statements []Statement}/Filter.Match(Node)/
│                             #   .Traversal()/.Narrowing() — the shared, triple-shaped
│                             #   node-selection type every VISION.md Filtering-section command
│                             #   and arc serve's MCP tools consume (specs/006-arc-grep-content-
│                             #   search, specs/027-triple-filter-model research.md D1-D3).
...
**Repository-context resolution is asymmetric across the use-cases, deliberately.** `internal/app/graph` and `internal/app/lint` probe git through their own `port.VCS`; `internal/app/ctrl` does not — `cmd/arc/ctrl/init.go` resolves the Parent Repository and the target's ignore status through `internal/adapter/git` and passes them into `service.Init` as plain `kernel.InitOpts` values. Two forces point the same way. `arc init`'s target may not exist yet, and both answers are needed *before* it is created, so a refusal never leaves a half-made directory behind — detection therefore cannot happen inside a use-case that only runs once its root is resolved. And detection is an environment probe, not a business rule: every decision taken from the answers (refuse, use the parent repository, refuse as ignored) still lives in the service, which is where Principle III actually draws the line. The alternative — widening `ctrl`'s port with three detection methods, and every fake with it — was considered and rejected (`specs/031-init-existing-git-repo`, research D6). Read the asymmetry as this trade-off, not as an oversight.
...
| **Arc State Directory** | The `.arc/` directory holding tool-managed graph state. It is **version-controlled**: `.arc/.gitkeep` and `.arc/.gitignore` are tracked, and `.arc/config.yml` travels with the repository, so a clone of a graph *is* a graph with no setup step (`specs/034-serve-dir-public-state`, research D3). The exclusion that remains applies to the **Arc Cache Directory** only, and is expressed twice: a `cache/` rule in the tracked `.arc/.gitignore`, which is the rule that survives a clone, plus a self-ignoring `*` in `.arc/cache/.gitignore`, which still holds if the parent rule is deleted. Both rules live *inside* `.arc/`, so `arc` still never creates, reads or appends to an ignore file it does not own — which is what lets a graph be added to a project that already has ignore rules of its own (`specs/031-init-existing-git-repo`, research D4). Its presence is what distinguishes an initialized Graph Root from an empty directory; no code path reads its `.gitignore`'s *content*, which is why a graph created before this layout keeps working untouched. |
| **Arc Cache Directory** | The `.arc/cache/` directory: the reserved home for machine-local state that is reproducible and disposable — derived indexes, probe artifacts, anything a different machine would simply regenerate. Never tracked, protected by the two independent rules the **Arc State Directory** describes. Modelled by `arcCacheDir` in `internal/app/ctrl/service` (no kernel type: it is a layout constant, not a domain value). Because its only file ignores itself, git cannot materialise the directory, so **it is absent in a fresh clone** — a deliberate consequence, not a defect, since cache content is machine-local by definition and the surviving `cache/` rule in the tracked parent already protects whatever a future writer creates (`specs/034-serve-dir-public-state`, data-model I6). `arc init` creates it; this feature deliberately gives it no producer (FR-015). |
...
| **Filter** | The optional, composable node-selection criteria (`Statements []Statement`, ANDed) shared by every VISION.md Filtering-section command and by `arc serve`'s MCP tools; a zero-value `Filter{}` matches every node. `Filter.Match(Node) bool` tests every statement against a node's attribute facts, its synthesized `(id, "type", Type)` fact, and its edge facts uniformly (spec FR-004/FR-005). `Filter.Traversal()`/`Filter.Narrowing()` partition `Statements` into traversal constraints (gate which edges `arc subgraph`/`subgraph_get`/`context_retrieve` BFS follows) versus ordinary narrowing statements (gate which reached, non-seed candidates survive into the result) — the split that lets a one-directional relation (e.g. `cites`) scope traversal without also requiring the reached node to independently re-satisfy it. `internal/core.Filter` (`specs/006-arc-grep-content-search`, `specs/027-triple-filter-model` research.md D1-D3) — `arc grep` was the first command to consume it; `--type`/`--tag`/`--attr` lower to specific `Statement` shapes that keep their pre-existing CLI contract (research.md D4). |
| **Statement** | One clause of a `Filter`: independently-wildcardable `Source`/`Predicate`/`Target` `Matcher` constraints on a triple **Fact** (an attribute, synthesized type, or edge fact). `Statement.IsTraversalConstraint()` reports whether `Source`/`Target` are both wildcard (`Predicate` alone constrained) — such a statement is a traversal constraint (research.md D3). `internal/core.Statement`, `specs/027-triple-filter-model` data-model.md. |
| **Matcher** | One position's constraint within a `Statement`: `Values` (OR-set, case-insensitive equality) and `Patterns` (OR-set, regexp) — either or both may be populated, and a zero-value `Matcher{}` is a wildcard matching every string. Generalizes the pre-existing `--attr name=value`/`name~=pattern` pair into one reusable type applied uniformly to `Source`, `Predicate`, and `Target`. `internal/core.Matcher`, `specs/027-triple-filter-model` research.md D1. |
| **Fact** | One `(Property, Value string)` pair carried by a node — its synthesized `type` fact, an attribute fact, or an edge fact — that satisfied at least one `Filter` `Statement`. `Filter.MatchingFacts(Node) []Fact` walks the same three fact sources `Filter.Match` already tests (`Statement.match`'s per-statement short-circuit becomes a per-fact collection instead), deduplicated by `(Property, Value)` and sorted for deterministic output; a node's `MatchingFacts` is non-empty iff `Match` is true for at least one statement. No `Source`/node-id field — a `Fact` describes *what* matched on a node, not *which* node; the owning node's id is carried separately by `kernel.MatchEntry`, `node_match`'s MCP-rendered row. `internal/core.Fact`, `specs/028-node-match-filter` data-model.md. |
...
| **Parent Repository** | The innermost git repository enclosing a target directory, found by the same upward search git itself performs (`git rev-parse --show-toplevel`), so `arc`'s judgement about "am I inside a project" agrees by construction with what every other tool in that project sees. `arc init` refuses to run when one exists — creating a repository inside a repository is the harmful default — and `--skip-git-init` inverts that into "add the graph to this repository instead", committing into its history rather than starting a new one. Reported as `InitResult.Repository` in both modes, in the caller's own path spelling so it stays comparable to the graph root (`specs/031-init-existing-git-repo`). |
| **Initialization Footprint** | The precise set of paths one `arc init` run put on disk: the files it wrote, in write order, and the directories it had to create to write them. It is what makes initialization safe in a directory `arc` does not exclusively own — it is the explicit pathspec `git add`/`git commit` are given, so the initial commit can never sweep in a user's unrelated modified, staged or untracked work, and it is the exact set rollback removes on failure, so a pre-existing file or directory is never deleted. It replaced a static "every path the layout could have written" list, which was safe only while an empty target was guaranteed — a guarantee `--skip-git-init` deliberately removes (`specs/031-init-existing-git-repo`, FR-013/FR-014/FR-018/FR-019). |
