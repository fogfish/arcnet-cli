...
│   ├── upgrade.go           # `arc upgrade [--dry-run]` command: top-level sibling of init — the same
│   │                         #   operation over a graph's built-in vocabulary, one for a new graph and
│   │                         #   one for an existing one; calls internal/app/ctrl.Upgrade
...
This project uses **bare top-level verbs** (`arc init`, `arc apply`, `arc upgrade`, `arc list`, ...), not noun-verb nesting — permitted by ADR 002 DS-01 because the entire tool operates on exactly one kind of subject, a knowledge graph. Every subcommand follows this convention without exception. The sole exception to "bare" is `arc apply schema`, a child of `arc apply` attached for naming consistency with `arc apply <patch.md>` even though its business logic and conceptual home (schema/config management) lives in `cmd/arc/ctrl`, not `cmd/arc/graph` (specs/018-apply-schema-patch, per explicit user direction).

## Compatibility Record

Breaking changes to **graphs already in the field** — as distinct from changes to the CLI surface — are recorded here, because a graph is data a user owns and the tool cannot silently rewrite it.

### `specs/023-core-vocabulary-conformance` — the merge vocabulary closed at six

**What breaks.** `internal/core.MergeOp` lost its seventh inhabitant, `validatedOverwrite`, and `schema/service.validMergeOps` no longer accepts it. Every graph seeded by an earlier release declares it on its own `_schema/Property/scoreZ.md` and `scoreC.md`, so **every command that reads such a graph hard-fails until `arc upgrade` runs.** There is no lenient-reader grace period: keeping the seventh value live is the conformance violation, not a workaround for one (CORE §9.3 fixes the menu at six; §14 requires profile predicates to draw from it).

**Why it was not softened.** A lenient reader for one release contradicts the requirement that the menu be exactly six, and leaves the non-conformant value in graphs indefinitely because nothing would ever force the migration. Silently rewriting the value on read mutates a user's graph without consent. Both were considered and rejected.

**What makes it survivable**, and what any future change of this shape must reproduce:

1. **The remedy ships before the break.** `arc upgrade` was implemented, tested, and merged in the phase *before* the gate tightened — deliberately inverting user-story priority, so that no release ever exists in which a graph can fail to load with no way forward.
2. **The error names the remedy.** `schema/service.ErrSchemaMergeInvalid` names the document, the offending value, the six legal values, **and `arc upgrade`**. This message is the first thing an existing user sees after upgrading the binary; without the fourth element the failure is a dead end.
3. **The remedy runs on the broken state.** `arc upgrade` writes the corrected vocabulary *before* it resolves anything (`ctrl/service.Upgrade`, contract C3.2). Resolving first would make the command that removes the offending declaration unable to run on any graph carrying it — a deadlock, closed structurally rather than by a special case.

**What does not break.** Every type-contract change in the same feature is a strict *relaxation* — the universal base type stopped requiring `published`/`created`, `Timeline` stopped requiring `granularity`/`period`, `Reference` stopped requiring `ref`/`relevance` — so no node that lint-ed clean before can start failing. Content is never rewritten: prose that accumulated under the old `append` behaviour is reported for review and left exactly as it is, because the boundary between original and appended text is unrecoverable.
...
| **Built-in Vocabulary** | The fixed set of Predicate and Type Schema Nodes `arc init` seeds into every new graph — `internal/app/schema/kernel.CorePredicateDefs` and `.CoreTypeDefs`, rendered to disk by `schema/service.Seed()`. It is ARCNET-CORE's own §9/§10/§11 vocabulary, not a starting point to be customized in place: **Schema Upgrade** replaces a built-in document outright, including one an author has hand-edited, while preserving every `_schema/` document that is not part of the set. `Seed()`'s byte-exact output is pinned by a golden snapshot (`internal/app/schema/service/testdata/golden/schema/`) so a change to what every new graph contains is reviewed as rendered Markdown rather than as a diff of Go map literals (`specs/023-core-vocabulary-conformance`, contract C2.1). |
| **Merge Vocabulary** | CORE §9.3's **closed set of six** merge operations — `immutable`, `union`, `firstWriteWin`, `fillIfEmpty`, `lastWriteWin`, `append` — enumerated by `internal/core.MergeOp`. Closed means exactly six: a Predicate Schema Node declaring anything else is rejected outright by `schema/service.Resolve` with `ErrSchemaInvalid`, which names the document, the offending value, the six legal values, and `arc upgrade` as the remedy. The tool's earlier seventh operation `validatedOverwrite` was **deleted, not deprecated** (`specs/023-core-vocabulary-conformance` FR-001/FR-004), which is why every graph seeded by a release before it must run a **Schema Upgrade** before any other command will load it. |
...
| **Schema Upgrade** | The single, explicitly invoked `arc upgrade` step that replaces an existing graph's **Built-in Vocabulary** documents with this release's, in exactly one `graph(migrate):` commit. Necessary because every correction in `specs/023-core-vocabulary-conformance` is a *retraction* — dropping a required predicate, changing a `merge` — and `arc apply schema`'s combining path can express none of them (`required`/`optional` are `union`, `role`/`merge` are `immutable`). Its execution order is normative rather than incidental (contract C3.2): it writes the corrected seed **before** it resolves anything, so it can run on a graph whose own `scoreZ.md` still declares the retired seventh merge value and would otherwise fail to load. Leaves every file outside `_schema/` and every non-built-in file inside it untouched, reports content nodes whose first-fixed prose shows evidence of prior accumulation without repairing any of them, and creates no commit at all when the graph is already current. `internal/app/ctrl/service/upgrade.go`, `internal/app/ctrl/kernel.UpgradeResult`. |
...
| **Entity/Resource Node** | A node of kind `Entity` or `Resource` — CORE's fixed kinds for, respectively, a subject occurring in sources and an anonymous fragment of an *ingested* document's content (`Required: text, tags, mentionedIn`; `Optional: notes`). A `Resource` is tag-classified staging ground: content worth keeping that does not yet warrant its own domain type, from which a recurring pattern is later promoted into one. It is **not** an external work the graph points to — that is a **Reference** (CORE §11.4/§11.6 v0.11, `specs/022-reference-type-folders`). Both kinds merge across multiple contributing patches; each predicate present on either kind reconciles by its own declared Merge Behavior, not by the node's own kind. |
| **Reference Node** | A node of kind `Reference` — an external work the graph points to but has **not** ingested, or a topic/area tracked for reading or research (CORE §11.6 v0.11). Records only enough to identify, locate, and justify keeping the pointer: `Required: title`; `Optional: url, authors, year, doi, ref, relevance, status, isCitedBy, notes, indexed`; `rdfs:subClassOf:: [[Node]]`. CORE §11.6 v0.11's normative `Class` block requires the title alone — `specs/023-core-vocabulary-conformance` F3 adopts it, superseding spec 022's recorded Clarification, so §11.6's own worked example conforms to the type it illustrates. Filed at `Reference/<id>.md`. Distinct from a **Source Node** (a document the graph *has* ingested, and the provenance origin other nodes derive from) and from an **Entity/Resource Node** (a fragment *of* an ingested document). Introduced by `specs/022-reference-type-folders`. |
