...
| **Statement** | One clause of a `Filter`: independently-wildcardable `Source`/`Predicate`/`Target` `Matcher` constraints on a triple **Fact** (an attribute, synthesized type, or edge fact — not a persisted Go type). `Statement.IsTraversalConstraint()` reports whether `Source`/`Target` are both wildcard (`Predicate` alone constrained) — such a statement is a traversal constraint (research.md D3). `internal/core.Statement`, `specs/027-triple-filter-model` data-model.md. |
