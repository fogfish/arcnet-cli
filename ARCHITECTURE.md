...
| **Type Schema Node** | A `Class`-typed node at `_schema/Class/<name>.md` (CORE §9.2, renamed from `_schema/nodes/<kind>.md`, then from `_schema/types/<name>.md` by `specs/022-reference-type-folders`): an optional `merge` attribute (the arc apply-specific bridge field beyond CORE's own documented shape, spec 011 FR-015 — retained only for schema-document-shape continuity on built-in/auto-registered documents, never validated as mandatory, spec 012 FR-020, Bugfix 018/BUG-001), mandatory descriptive body, and zero or more `required`/`optional` predicate-name bullets a conforming instance must/may carry — decoded into `core.TypeDef`. Its own `@id`/name and every node's `@type` value referencing it MUST be CamelCase — begin with an uppercase letter (`specs/019-camelcase-node-types`); `arc apply` rejects a non-conforming patch H1/`@type` outright, and `arc lint`'s `RuleTypeCase` flags a non-conforming pre-existing type definition or reference. Seeded for ARCNET-CORE's five fixed content types (`Source`/`Entity`/`Resource`/`Timeline`/`Reference`) plus `Node` and `Property`/`Class` themselves by `arc init`; auto-registered (`merge: union`, empty `required`/`optional`, a placeholder description) the first time `arc apply` encounters an unrecognized type; never overwritten automatically once present. Replaces the existence-only Node-Kind Schema Document spec 005 introduced. |
...
| **Reference Node** | A node of kind `Reference` — an external work the graph points to but has **not** ingested, or a topic/area tracked for reading or research (CORE §11.6 v0.11). Records only enough to identify, locate, and justify keeping the pointer: `Required: title, ref, relevance`; `Optional: url, authors, year, doi, status, isCitedBy, notes`; `rdfs:subClassOf:: [[Node]]`. Filed at `Reference/<id>.md`. Distinct from a **Source Node** (a document the graph *has* ingested, and the provenance origin other nodes derive from) and from an **Entity/Resource Node** (a fragment *of* an ingested document). Carries `merge: firstWriteWin`, inherited from `Resource`'s pre-v0.11 definition — the op belonged to the external-work semantics, which moved here wholesale. Introduced by `specs/022-reference-type-folders`. |
