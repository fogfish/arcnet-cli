...
| **Arc State Directory** | The `.arc/` directory holding tool-managed local state, never versioned alongside graph content. The exclusion lives *inside* it, as `.arc/.gitignore` containing a single `*` rule, rather than in a `.gitignore` at the Graph Root: a `*` there excludes the directory's entire contents including the rule file itself, so `arc` never creates, reads or appends to an ignore file it does not own — which is what lets a graph be added to a project that already has ignore rules of its own (`specs/031-init-existing-git-repo`, research D4). Its presence is what distinguishes an initialized Graph Root from an empty directory. |
